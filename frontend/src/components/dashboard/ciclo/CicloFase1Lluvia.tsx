    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-end">
        <div className="space-y-1">
          <h3 className="text-2xl font-black font-display uppercase tracking-tight text-pclr4 dark:text-pdclr4">Lluvia de Ideas</h3>
        ...
        {cicloActivo.fase_actual === 1 && !readOnlyOverride && (
          <button 
            ...
            className="p-2 text-pclr12 font-bold uppercase rounded-2xl shadow-lg text-shadow-lg hover:scale-105 active:scale-95 transition-all text-base tracking-widest"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPropuestas.map(p => (
          <div 
            key={p.id} 
            className={`p-4 rounded-[1.5rem] bg-pclr1 dark:bg-pdclr1 border-2 transition-all shadow-sm flex flex-col justify-between gap-4 relative overflow-hidden ${
              p.preseleccionada || p.es_grupal_global ? "shadow-lg border-opacity-100" : "opacity-90 border-pclr13 dark:border-pdclr13"
            }`} 
            style={{ 
          ...
          >
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-start">
                ...
                {(p.preseleccionada || p.es_grupal_global) && (
                  <span className="px-2 py-0.5 bg-pclr6 text-pclr12 text-[1em] font-black rounded-md uppercase shadow-sm">
            ...
            {canManage && cicloActivo.fase_actual === 1 && !p.es_grupal_global && (
              <div className="pt-4 border-t border-pclr13 dark:border-pdclr13 flex justify-between items-center">
                <span className="text-[0.9em] font-black uppercase text-pclr7">Gestión Admin</span>
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl text-[0.9em] font-bold text-shadow-lg uppercase transition-all shadow-sm ${
                      p.preseleccionada ? 'bg-pclr3 text-pclr7 hover:bg-pclr3' : 'text-pclr12 hover:brightness-110'
                    }`}
                  ...
                  <button 
                    ...
                    className="px-3 py-2 rounded-xl text-[0.9em] font-black uppercase bg-pclr10 dark:bg-pdclr10 text-pclr12 dark:text-pdclr12 hover:brightness-110 transition-all shadow-sm"
