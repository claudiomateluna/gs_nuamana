...
describe('generateThemeCSS', () => {
  it('emits a minified :root block with all 18 default variables in order', () => {
    const css = generateThemeCSS(DEFAULTS);
    expect(css).toBe(
      ':root{--clr1:#FFFFFF;--clr2:#95a5a6;--clr3:#333333;--clr4:#1d1d1d;--clr5:#2c3e50;--clr6:#3eb34b;--clr7:#cb3327;--clr8:#ffc41d;--clr9:#f8f9fa;--clr10:#e9ecef;--dclr1:#121212;--dclr2:#b0b0b0;--dclr3:#1e1e1e;--dclr4:#0a0a0a;--dclr5:#1a2a3a;--dclr6:#2d8a3a;--dclr7:#a3281f;--dclr8:#e6b01a;}',
    );
  });
  ...
  it('reflects a custom clr7 override instead of the default', () => {
    ...
    expect(css).toContain('--dclr8:#e6b01a');
    expect(css).toContain('--clr1:#FFFFFF');
  });

  it('always wraps the block in :root{...} and emits each of the 18 variable names exactly once', () => {
    const css = generateThemeCSS(DEFAULTS);
