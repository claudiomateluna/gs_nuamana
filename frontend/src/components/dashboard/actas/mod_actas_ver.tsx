        ...
        {grupalAcuerdos.length > 0 && (
          <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-blue-800 dark:text-blue-300">📋 Fichas de Actividad</p>
                  ...
                  {acta.acuerdos?.map((a: any, i: number) => (
                    <div key={i} className="p-2 bg-blue-50/30 dark:bg-clr3/40 rounded-[0.5rem] border border-blue-100 dark:border-clr4 relative group shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-4 pt-2 border-t border-blue-100/50 text-[0.8em]">
                        ...
                        {a.es_actividad_grupal && <span className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full font-bold">👥 Grupal</span>}
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {fichasPorAcuerdo[i].map((ficha: Articulo) => (
                            <a key={ficha.id} href={`/blog/actividades/${ficha.slug}`} target="_blank" rel="noopener noreferrer"
                               className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold hover:bg-blue-200">
                              📋 {ficha.titulo}
                            </a>
                          ))}
                        </div>
                      ...
                      {a.es_actividad_grupal && editingAcuerdoIdx !== i && (
                        <button 
                          ...
                          className="mt-2 px-3 py-1 bg-clr6 text-white rounded-full text-xs font-bold hover:brightness-110 transition-all"
                        >
                        ...
                        <div className="mt-3 p-3 bg-white dark:bg-clr3 rounded-xl border-2 border-clr6">
                          ...
                          <div className="flex gap-2 mt-3">
                            <button 
                              onClick={saveFichas} 
                              disabled={saving}
                              className="px-4 py-2 bg-clr6 text-white rounded-xl text-xs font-bold hover:brightness-110 disabled:opacity-50"
                            >
