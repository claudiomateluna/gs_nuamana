...
import Step0_SeleccionActividad from './autorizacion/Step0_SeleccionActividad'
import Step1_DatosPersonales from './autorizacion/Step1_DatosPersonales'
import Step2_DatosGrupo from './autorizacion/Step2_DatosGrupo'
import Step3_ContactosEmergencia from './autorizacion/Step3_ContactosEmergencia'
import Step4_PrevisionSalud from './autorizacion/Step4_PrevisionSalud'
import Step5_AlergiasIntolerancias from './autorizacion/Step5_AlergiasIntolerancias'
import Step6_EnfermedadesCronicas from './autorizacion/Step6_EnfermedadesCronicas'
import Step7_SaludMental from './autorizacion/Step7_SaludMental'
import Step8_HistorialClinico from './autorizacion/Step8_HistorialClinico'
import Step9_CondicionesRecientes from './autorizacion/Step9_CondicionesRecientes'
import Step10_ContactosRecientes from './autorizacion/Step10_ContactosRecientes'
import Step11_AntecedentesVacunas from './autorizacion/Step11_AntecedentesVacunas'
import Step12_SaludGineco from './autorizacion/Step12_SaludGineco'
import Step13_SaludDental from './autorizacion/Step13_SaludDental'
import Step14_NecesidadesEspeciales from './autorizacion/Step14_NecesidadesEspeciales'
import Step15_RegulacionEmocional from './autorizacion/Step15_RegulacionEmocional'
import Step16_AutorizacionParticipacion from './autorizacion/Step16_AutorizacionParticipacion'
import Step17_AutorizacionImagen from './autorizacion/Step17_AutorizacionImagen'
import Step18_FirmaDigital from './autorizacion/Step18_FirmaDigital'

interface Props {
  isOpen: boolean
  onClose: () => void
  perfil: any
  actividadInitial?: any
  onSuccess?: () => void
}
...
export default function DashModAutorizacionWizard({ isOpen, onClose, perfil, actividadInitial, onSuccess }: Props) {
  ...
  const [actividadSelected, setActividadSelected] = useState<any>(actividadInitial || null)
  const [apoderadoData, setApoderadoData] = useState<any>(null)
  ...
  const handleFinalSave = async () => {
    if (!actividadSelected) return alert('Debes seleccionar una actividad.')
    setSaving(true)
    try {
      ...
      finalValidColumns.forEach(col => {
        if (formData[col] !== undefined) {
          ...
          if (col === 'estatura_m' || col === 'peso_kg') {
            ...
            if (col === 'estatura_m' && value > 9.99) value = 9.99;
            if (col === 'peso_kg' && value > 999.99) value = 999.99;
          }
      ...
      alert('¡Autorización firmada con éxito!')
        ...
        <div className="p-4 bg-zinc-50 dark:bg-black/20 border-t border-zinc-100 dark:border-clr4 flex justify-between items-center">
          ...
          {step < 18 ? (
            <button 
              onClick={handleNext} 
              disabled={!canAdvance}
              className={`px-2 py-2 bg-clr7 text-white font-black uppercase rounded-xl shadow-lg text-[1em] tracking-widest ${!canAdvance ? 'opacity-50 cursor-not-allowed' : 'hover:brightness-110 active:scale-95'}`}
            >
              Siguiente →
            </button>
          ) : (
            <button onClick={handleFinalSave} disabled={saving || !formData.firma} className="px-10 py-3 bg-green-600 text-white font-black uppercase rounded-xl shadow-lg text-[9px] tracking-widest">✍️ FINALIZAR</button>
          )}
        </div>
      </div>
    </div>
