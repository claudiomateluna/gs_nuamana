  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 font-body">
      ...
      {!isDirectivo && !readOnlyOverride && (
        <div className="space-y-4">
          <div className="space-y-1">
            <h3 className="text-2xl font-black font-display uppercase tracking-tight text-pclr4 dark:text-pdclr4">Mi Progresión Personal</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {actividadesAsistidas.map(act => (
              <div key={act.id} className="p-2 bg-pclr1 dark:bg-pdclr1 rounded-[1rem] border-2 border-pclr13 dark:border-pdclr13 flex items-center justify-between gap-4 group hover:border-pclr14 transition-all">
                <div className="flex-1">
                  ...
                  <h4 className="font-bold uppercase text-pclr4 dark:text-pdclr4 line-clamp-1">{act.titulo}</h4>
                </div>
                {cicloActivo.fase_actual === 5 && (
                  <button 
                    ...
                    className="px-2 py-1 bg-pclr2 text-pclr12 text-[0.8em] font-black uppercase rounded-[0.6rem] shadow-lg hover:scale-105 active:scale-95 transition-all tracking-widest border-none"
                  >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {nnjEvaluables.map(nnj => (
              <div key={nnj.id} className="p-2 bg-pclr1 dark:bg-pdclr1 rounded-[1rem] border-2 border-pclr13 dark:border-pdclr13 shadow-sm space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-pclr3 dark:bg-pdclr3 flex items-center justify-center font-black text-xs">
                    {nnj.nombres[0]}{nnj.apellidos[0]}
                  </div>
                  <div className="flex-1">
                    <h5 className="text-[1em] font-bold uppercase text-pclr4 dark:text-pdclr4 leading-none" style={{ color: unitColor }}>{nnj.nombres}</h5>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {nnj.actividades.map((act: CicloPropuesta) => (
                      <button 
                        ...
                        className={`px-2 py-2 rounded-xl text-[0.9em] font-bold transition-all border ${
                          cicloActivo.fase_actual === 5 
                            ? 'bg-pclr3 dark:bg-pdclr1 hover:bg-pclr10 hover:text-pclr12 border-pclr13 dark:border-transparent' 
                            : 'bg-pclr3 opacity-50 cursor-not-allowed border-transparent'
                        }`}
                    ...
                    {nnj.actividades.length === 0 && (
                      <p className="text-[1em] italic text-pclr4">Sin asistencias registradas.</p>
                    )}
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full py-2 bg-pclr2 text-pclr12 text-[0.9em] font-bold uppercase rounded-xl shadow-md hover:scale-105 transition-all tracking-widest flex items-center justify-center gap-2 border-none"
                >
      ...
      {!isDirectivo && !readOnlyOverride && actividadesAsistidas.length > 0 && (
        <div className="space-y-4 bg-pclr1 dark:bg-pdclr1 p-2 rounded-[1rem] border border-pclr13 dark:border-pdclr13 shadow-sm">
      ...
      {canManage && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pt-8 border-t border-pclr13 dark:border-pdclr13">
          <div className="space-y-1">
            <h3 className="text-2xl font-black font-display uppercase tracking-tight text-pclr4 dark:text-pdclr4" style={{ color: unitColor }}>Evaluación del Ciclo</h3>
          ...
          {canManage && cicloActivo.fase_actual === 5 && !readOnlyOverride && (
            <button 
              onClick={cerrarCiclo}
              className="px-3 py-3 bg-pclr10 text-pclr12 font-black uppercase rounded-2xl shadow-lg hover:bg-pclr10 transition-all text-[1em] flex items-center gap-2 border-none"
            >
      ...
      <div className="space-y-2 bg-pclr1 dark:bg-pdclr1 p-2 rounded-[1rem]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {propuestas.filter(p => p.seleccionada && !p.es_actividad_programada).map(p => (
            <div key={p.id} className="p-2 rounded-[0.6rem] bg-pclr3 dark:bg-pdclr3 border-2 border-dashed border-pclr13 dark:border-pdclr13">
              <div className="flex justify-between items-start mb-2">
                <h5 className="text-[1em] font-bold uppercase text-pclr4 dark:text-pdclr4">{p.titulo}</h5>
                {p.evaluacion ? (
      ...
      <div className="bg-pclr1 dark:bg-pdclr1 p-2 rounded-[1rem] border border-pclr13 dark:border-pdclr13 shadow-sm space-y-6">
          ...
          <div className="space-y-2">
            ...
            {canManage && cicloActivo.fase_actual === 5 ? (
              <textarea 
                ...
                className="w-full p-4 rounded-2xl border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 font-body text-sm min-h-[150px]"
              />
            ) : (
              <p className="p-4 rounded-2xl border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 font-body text-sm min-h-[150px] italic opacity-80 whitespace-pre-wrap">
        ...
        {canManage && cicloActivo.fase_actual === 5 && (
          <div className="flex justify-end pt-4">
            <button 
              ...
              className="px-6 py-3 text-pclr12 font-black uppercase rounded-xl shadow-lg hover:scale-105 active:scale-95 transition-all text-xs tracking-widest border-none text-shadow-lg"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 pt-8 border-t border-pclr13 dark:border-pdclr13">
        {/* Resumen de Actividades */}
        <div className="lg:col-span-1 space-y-2 bg-pclr1 dark:bg-pdclr1 p-2 rounded-[1rem] border border-pclr13 dark:border-pdclr13 shadow-sm">
          <h4 className="font-bold uppercase text-shadow-lg" style={{ color: unitColor }}>Resumen Estadístico</h4>
          
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-pclr3 dark:bg-pdclr3 rounded-2xl">
          ...
          <div className="pt-6 border-t border-pclr13 dark:border-pdclr13">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-2 bg-pclr1 dark:bg-pdclr1 rounded-[1rem]">
          ...
          {objetivosTrabajados.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
              {Object.entries(
                ...
                <div key={area} className="p-2 rounded-[1rem] bg-pclr1 dark:bg-pdclr1 border-2 border-pclr13 dark:border-pdclr13 shadow-sm space-y-4 font-body">
                  <div className="flex justify-between items-center border-b border-pclr13 dark:border-pdclr13 pb-2">
                    <span className="text-sm font-black uppercase tracking-widest" style={{ color: unitColor }}>{area}</span>
                    <span className="w-10 h-10 rounded-full bg-pclr3 dark:bg-pdclr3 flex items-center justify-center text-sm font-black">{objs.length}</span>
                  </div>
            ...
            <div className="py-20 text-center border-4 border-dashed border-pclr13 dark:border-pdclr13 rounded-[3rem] opacity-30 bg-pclr3 dark:bg-pdclr3">
