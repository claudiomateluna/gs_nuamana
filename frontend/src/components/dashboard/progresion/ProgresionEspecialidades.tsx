    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="space-y-4">
          <div className="border-b pb-4 border-pclr13 dark:border-pdclr13 flex justify-between items-center">
            <div>
              <h3 className="font-bold uppercase text-[1.5em] text-pclr4 dark:text-pdclr4 flex items-center gap-2">
                🎓 Panel de Tutorías y Supervisiones
              </h3>
              <p className="text-[0.9em] font-bold text-pclr7">
          ...
          {supervisedSpecialtyProgress.length === 0 ? (
            <div className="p-8 rounded-[2rem] bg-pclr3 dark:bg-pdclr3 border border-pclr13 dark:border-pdclr13 text-center">
              <span className="text-4xl mb-2 block">🎓</span>
              <h4 className="font-bold text-[1.1em] text-pclr4 dark:text-pdclr4">No tienes especialidades asignadas para supervisar</h4>
              <p className="text-[0.9em] text-pclr7 dark:text-pdclr7 mt-1 max-w-md mx-auto">
              ...
              {supervisedSpecialtyProgress.map(({ ep, completedCount, totalCount, pct, specialistName }) => {
                ...
                return (
                  <div
                    ...
                    className="p-6 rounded-[2rem] bg-pclr1 dark:bg-pdclr1 border border-pclr13 dark:border-pdclr13 hover:border-pclr13 dark:hover:border-dclr7 shadow-lg cursor-pointer hover:shadow-xl transition-all relative overflow-hidden group border-l-[6px]"
                  ...
                  >
                    <div className="flex gap-4 items-start mb-3">
                      <img
                        ...
                        className="w-16 h-16 object-contain shadow-md rounded-2xl bg-pclr3 dark:bg-pdclr3 p-1.5 border dark:border-pclr1 shrink-0"
                        loading="lazy"
                        decoding="async"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5 mb-1">
                          ...
                          <span className="text-[0.8em] font-bold text-pclr7 uppercase bg-pclr3 dark:bg-pdclr3 px-2 py-0.5 rounded-full">
                            {getPhaseLabel(ep.fase)}
                          </span>
                        </div>
                        <h4 className="font-bold text-[1.2em] text-pclr4 dark:text-pdclr4 uppercase leading-tight group-hover:text-pclr4 dark:group-hover:text-pclr7 truncate">
                          {name}
                        </h4>
                        <p className="text-[0.8em] font-bold text-pclr7 dark:text-pdclr7 mt-1">
                          👤 {specialistName}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-[0.8em] font-bold">
                        <span className="text-pclr7">Progreso Actividades</span>
                        <span style={{ color }}>{completedCount}/{totalCount} ({pct}%)</span>
                      </div>
                      <div className="w-full bg-pclr3 dark:bg-pdclr3 rounded-full h-2.5 overflow-hidden">
                        <div
                    ...
                    {/* Quick status badges */}
                    <div className="mt-4 pt-3 border-t border-pclr13 dark:border-pdclr13 flex justify-between items-center text-[0.8em] font-bold text-pclr7">
                      <span>Iniciada {ep.fecha_inicio}</span>
                      {ep.estado === 'completado' ? (
                        <span className="text-pclr6 uppercase">Completada 🎉</span>
                      ) : (
                        (ep.especialidades_actividades || []).some((a: EspecialidadActividad) => !a.completada && a.evidencia_texto) && (
                          <span className="text-pclr4 animate-pulse uppercase font-extrabold">Pendiente Aprobación 🔔</span>
                        )
                      )}
                    </div>
                  </div>
    ...
    <div className="space-y-10 animate-in fade-in duration-300">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center border-b pb-4 border-pclr13 dark:border-pdclr13">
          <div>
            <h3 className="font-bold uppercase text-[1.5em] text-pclr4 dark:text-pdclr4">Especialidades en Desarrollo</h3>
            <p className="text-[0.9em] font-bold text-pclr7">Tus proyectos personales y especialidades activas.</p>
          </div>
        ...
        {loadingEspecialidades ? (
          ...
          <div className="p-2 rounded-[1rem] bg-pclr3 dark:bg-pdclr3 border border-pclr13 dark:border-pdclr13 text-center">
            <span className="text-4xl mb-2 block">✨</span>
            <h4 className="font-bold text-[1.1em] text-pclr4 dark:text-pdclr4">¡Aún no tienes especialidades activas!</h4>
            <p className="text-[0.9em] text-pclr7 dark:text-pdclr7 mt-1 max-w-md mx-auto">
                  ...
                  <div className="flex gap-4 items-start mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5 mb-1">
                        ...
                        {ep.estado === 'pausado' && (
                          <span className="text-[0.8em] font-extrabold text-pclr5 bg-pclr5 dark:bg-pdclr5 px-2 py-0.5 rounded-full border border-pclr5">
                            Pausada ⏸️
                          </span>
                        )}
                      </div>
                      ...
                      {ep.monitor_nombre && (
                        <p className="text-[0.8em] font-semibold text-pclr7 dark:text-pdclr7 mt-1">
      ...
      {canSeeAllTabs(perfil) && (
        <div className="space-y-6 pt-6">
          <div className="border-b pb-4 border-pclr13 dark:border-pdclr13">
            <h3 className="font-bold uppercase text-[1.5em] text-pclr4 dark:text-pdclr4">Catálogo de Especialidades</h3>
            <p className="text-[0.9em] font-bold text-pclr7">Explora las especialidades sugeridas oficiales para tu unidad.</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-2">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full p-4 pl-12 rounded-2xl border dark:border-pclr1 bg-pclr3 dark:bg-pdclr3 text-[0.9em] font-bold"
              />
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xl text-pclr7">🔍</span>
            </div>

            <select
              value={selectedFieldFilter}
              onChange={e => setSelectedFieldFilter(e.target.value)}
              className="p-4 rounded-2xl border dark:border-pclr1 bg-pclr3 dark:bg-pdclr3 text-[0.9em] font-bold min-w-[200px]"
            >
          ...
          {filteredCatalog.length === 0 ? (
            <p className="text-center py-10 text-pclr7 font-bold italic">No se encontraron especialidades que coincidan con los filtros.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCatalog.map(def => {
                ...
                return (
                  <div
                    key={def.id}
                    className="p-2 rounded-[1rem] bg-pclr1 dark:bg-pdclr1 border border-pclr13 dark:border-pdclr13 hover:border-pclr13 dark:hover:border-dclr7 shadow-md hover:shadow-xl hover:scale-[1.01] transition-all cursor-pointer flex flex-col justify-between group border-l-[6px]"
                    style={{ borderLeftColor: color, contentVisibility: 'auto' } as React.CSSProperties}
                  ...
                  >
                    <div>
                      ...
                      <div className="flex gap-2 items-center mb-3">
                        ...
                        />
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-[1.25em] text-pclr4 dark:text-pdclr4 uppercase leading-tight mb-2 group-hover:text-pclr4 dark:group-hover:text-pclr7 truncate">
                            {def.nombre}
                          </h4>
                          <p className="text-[0.9em] font-medium text-pclr7 dark:text-pdclr7 line-clamp-3 leading-relaxed">
                            {def.descripcion}
                          </p>
                        </div>
                      </div>
                    ...
                    {isOwner && !isNNJConAgenda(perfil) && (
                      <div className="mt-4 pt-3 border-t border-pclr13 dark:border-pdclr13 flex justify-end">
                        <span
