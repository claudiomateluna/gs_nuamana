...
const registroSchema = z.object({
  claveAutorizacion: z.string().min(1, 'La clave es requerida').refine(val => val === (process.env.NEXT_PUBLIC_REGISTRATION_KEY || '2005'), { message: 'Clave incorrecta' }),
...
function RegistroContent() {
  ...
  const onSubmit = async (data: z.infer<typeof registroSchema>) => {
    ...
    try {
      ...
      toast.info('¡Datos validados con éxito!')

      // Si es dirigente o guiadora, cerramos la sesión automática de Supabase
      if (data.rol === 'dirigente' || data.rol === 'guiadora') {
        await supabase.auth.signOut()
        toast.success('Registro completado. Tu cuenta de ' + data.rol + ' está en revisión por la directiva. Te avisaremos cuando sea activada.')
        window.location.href = '/login'
      } else {
        toast.info('¡Bienvenido a Nua Mana! Ya puedes acceder a tu panel.')
