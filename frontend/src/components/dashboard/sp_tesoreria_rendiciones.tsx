  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-[1em]">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-black font-display uppercase font-bold text-clr5 dark:text-dclr2">Rendiciones de Cuentas (DAF-FOR-03)</h2>
        {isAdmin && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {rendiciones.map(r => (
          <div key={r.id} className="bg-white dark:bg-black/10 p-6 rounded-[2.5rem] border border-zinc-100 dark:border-dclr10 shadow-sm hover:shadow-xl transition-all relative group">
            ...
            <div className="space-y-4">
              <div>
                ...
                <h3 className="text-[1.1em] font-bold uppercase leading-tight text-clr5 dark:text-dclr2 line-clamp-2 h-12">{r.motivo}</h3>
              </div>

              <div className="grid grid-cols-2 gap-2 text-[0.8em] border-t border-zinc-50 dark:border-dclr10 pt-4">
                <div>
              ...
              <div className="flex gap-2 pt-2 border-t border-zinc-50 dark:border-dclr10">
                <button onClick={() => onVer(r)} className="flex-1 py-2 bg-zinc-100 dark:bg-dclr1 rounded-lg text-[0.8em] font-bold uppercase hover:bg-clr5 hover:text-white transition-all">Ver Documento</button>
                {isAdmin && (
                  <button onClick={() => onDelete(r.id)} className="px-4 py-2 bg-zinc-100 dark:bg-dclr1 rounded-lg text-[0.8em] font-bold uppercase hover:bg-red-500 hover:text-white transition-all">Eliminar</button>
                )}
              </div>
            </div>
          </div>
