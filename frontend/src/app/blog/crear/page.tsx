...
export default function CrearArticuloPage() {
  ...
  if (!mounted || loading) return <div className="p-20 text-center font-body text-clr3 italic tracking-widest text-[0.8em] uppercase">Preparando el taller...</div>
  ...
  return (
    <div className="min-h-screen bg-clr7 dark:bg-dclr7 font-body transition-colors">
      ...
      <main className="max-w-[1080px] mx-auto px-4 py-32">
        <div className="bg-clr1 dark:bg-dclr1 rounded-[1rem] p-4 shadow-2xl border border-clr7 dark:border-dclr7 animate-in fade-in zoom-in duration-700">
          <h1 className="text-3xl font-black text-clr4 mb-8 uppercase tracking-tighter font-display border-b border-clr7 dark:border-dclr7 pb-4">Nueva Entrada</h1>
          
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="lg:col-span-3 space-y-4">
              <input {...register('titulo')} className="w-full text-3xl font-bold border-b border-clr7 dark:border-dclr7 focus:border-clr4 outline-none py-2 font-display bg-transparent" placeholder="Título del artículo..." required />
                  ...
                  <div className="mt-4 pt-4 border-t border-clr4 dark:border-dclr4 space-y-6">
                    <div className="flex justify-between items-end">
                      ...
                      {selectedObjsEd.length > 0 && (
                        <span className="px-3 py-1 bg-clr4 text-clr1 text-[0.9em] font-black rounded-full shadow-md animate-bounce">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className="w-full p-4 pl-4 rounded-2xl border bg-clr1 dark:bg-dclr1 text-[1em] font-bold focus:ring-2 ring-clr4 transition-all outline-none"
                      />
                    </div>
                                      ...
                                      <div className="flex items-center gap-2">
                                        {obj.texto_terminal && (
                                          <div className="relative group/tooltip flex items-center justify-center" onClick={(e) => { e.stopPropagation(); toast.info('🎯 OBJETIVO TERMINAL:\n\n' + obj.texto_terminal); }}>
                                            ...
                                            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:group-hover/tooltip:block w-80 max-w-[90vw] p-6 bg-clr2 text-clr1 text-[0.9em] font-bold rounded-[2rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] z-[9999] animate-in fade-in zoom-in duration-200 pointer-events-none text-center border border-clr2 flex flex-col items-center gap-3">
                                              <div className="flex flex-col items-center gap-1">
                                                <span className="text-clr4 uppercase tracking-widest block opacity-90 text-[0.8em] font-black">🎯 Objetivo Terminal</span>
                                              </div>
            ...
            <div className="space-y-4">
              <div className="p-2 bg-clr7 dark:bg-dclr7 rounded-2xl border border-clr7 dark:border-dclr7">
                ...
                {imageMode === 'url' ? (
                  ...
                  <div className="space-y-4 mb-4">
                    <button type="button" disabled={uploading} onClick={() => fileInputRef.current?.click()} className={`w-full py-4 border-2 border-dashed border-clr3 rounded-2xl flex flex-col items-center justify-center gap-2 hover:bg-clr1 transition-all ${uploading ? 'animate-pulse opacity-50' : ''}`}>
                      <span className="text-3xl">{uploading ? '⏳' : '📤'}</span>
                      <span className="text-[0.9em] font-black uppercase text-clr3">{uploading ? 'Subiendo...' : 'Seleccionar Archivo'}</span>
                    </button>
                    <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept="image/*" className="hidden" />
                    {currentImage && (
                      <button type="button" onClick={() => setValue('imagen_destacada', '')} className="text-[0.8em] font-black text-clr4 uppercase tracking-widest hover:underline block mx-auto">✕ Eliminar foto subida</button>
                    )}
                  </div>
                )}
                {currentImage && (
                  <div className="relative rounded-2x1 overflow-hidden shadow-lg border-2 border-clr1 dark:border-dclr1 animate-in fade-in zoom-in duration-300">
              ...
              <div className="p-2 bg-clr7 dark:bg-dclr7 rounded-2xl border border-clr7 dark:border-dclr7">
                <h3 className="text-[1em] font-black text-clr3 uppercase mb-6 tracking-widest font-display">Categorías</h3>
                <div className="space-y-4">
              ...
              <div className="p-2 bg-clr7 dark:bg-dclr7 rounded-2xl border border-clr7 dark:border-dclr7">
                <h3 className="text-[1em] font-black text-clr3 uppercase mb-2 tracking-widest font-display">Etiquetas</h3>
                <input {...register('etiquetas_input')} list="tags-list" className="w-full p-3 rounded-xl border text-sm bg-clr1 dark:bg-dclr1" />
              </div>

              <button type="submit" disabled={loading} className="btn-save w-full py-6 bg-clr4 text-clr1 font-black uppercase rounded-2xl shadow-2xl hover:brightness-110 active:scale-95 transition-all tracking-widest font-display text-sm">
