...
export default function DashModActaVer({ isOpen, onClose, acta }: DashModActaVerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-clr2 backdrop-blur-md z-[100] flex items-center justify-center p-2 xs:p-1 sm:p-2 md:p-3 lg:p-4 animate-in zoom-in-95 duration-300 text-[1em]">
      <div className="bg-clr1 dark:bg-dclr1 w-full sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg rounded-[1em] p-3 shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <header className="mb-4 border-b pb-2 flex flex-col md:flex-row md:items-center gap-3">
          <div className="w-20 h-20 bg-clr6 rounded-[1em] flex flex-col items-center justify-center text-clr1 shadow-lg shrink-0">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[0.8em] font-bold uppercase ${
                acta.confidencialidad === 'Pública' ? 'bg-clr6 text-clr6' : 'bg-clr5 text-clr5'
              }`}>
                🔒 {acta.confidencialidad}
              </span>
            </div>
            <h2 className="text-3xl font-black font-display uppercase tracking-tighter text-clr4 dark:text-dclr4">{acta.tipo}</h2>
        ...
        {grupalAcuerdos.length > 0 && (
          <div className="mb-4 p-3 bg-clr4 dark:bg-dclr4 rounded-xl border border-clr4 dark:border-dclr4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-clr4 dark:text-dclr4">📋 Fichas de Actividad</p>
        ...
        <div className="space-y-4">
          {/* RESUMEN AGENDA */}
          <section className="bg-clr7 dark:bg-dclr7 p-2 rounded-[0.5em] border border-clr7 dark:border-dclr7">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-10">
              <section className="space-y-4">
                ...
                <div className="space-y-4">
                  {acta.temas?.map((t: any, i: number) => (
                    <div key={i} className="pl-4 border-l-4 border-clr6 relative">
                      <div className="absolute -left-2 top-0 w-3 h-3 rounded-full bg-clr6 shadow-sm" />
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-black text-clr4 dark:text-dclr4 uppercase text-[1em] tracking-tight">{t.titulo}</h4>
                      ...
                      <div className="space-y-4 text-[1em]">
                        ...
                        {t.conclusiones && (
                          <div className="bg-clr6 p-3 rounded-xl border border-clr6">
              ...
              <section className="space-y-2">
                <h3 className="font-bold uppercase text-[0.8em] opacity-40 border-b pb-2 tracking-widest font-slab text-clr4 dark:text-dclr4">Acuerdos y Compromisos</h3>
                <div className="grid gap-2 text-[1em]">
                  {acta.acuerdos?.map((a: any, i: number) => (
                    <div key={i} className="p-2 bg-clr4 dark:bg-dclr4 rounded-[0.5rem] border border-clr4 dark:border-dclr4 relative group shadow-sm">
                      <div className="flex justify-between items-start mb-3">
                        <h4 className="font-bold uppercase text-clr4 dark:text-dclr4">{a.titulo}</h4>
                        <span className={`px-2 py-0.5 rounded-full text-[1em] ${
                          a.prioridad === 'Alta' || a.prioridad === 'Urgente' ? 'bg-clr4 text-clr4' : 'bg-clr4 text-clr4'
                        }`}>{a.prioridad}</span>
                      </div>
                      <p className="mb-4 font-bold">{a.descripcion}</p>
                      <div className="flex flex-wrap gap-4 pt-2 border-t border-clr4 text-[0.8em]">
                        ...
                        {a.es_actividad_grupal && <span className="px-2 py-0.5 bg-clr4 text-clr4 rounded-full font-bold">👥 Grupal</span>}
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {fichasPorAcuerdo[i].map((ficha: Articulo) => (
                            <a key={ficha.id} href={`/blog/actividades/${ficha.slug}`} target="_blank" rel="noopener noreferrer"
                               className="px-3 py-1 bg-clr4 text-clr4 rounded-full text-xs font-bold hover:bg-clr4">
                              📋 {ficha.titulo}
                            </a>
                          ))}
                        </div>
                      ...
                      {a.es_actividad_grupal && editingAcuerdoIdx !== i && (
                        <button 
                          ...
                          className="mt-2 px-3 py-1 bg-clr6 text-clr1 rounded-full text-xs font-bold hover:brightness-110 transition-all"
                        >
                      ...
                      {editingAcuerdoIdx === i && (
                        <div className="mt-3 p-3 bg-clr1 dark:bg-dclr1 rounded-xl border-2 border-clr6">
                          ...
                          <div className="flex gap-2 mt-3">
                            <button 
                              onClick={saveFichas} 
                              disabled={saving}
                              className="px-4 py-2 bg-clr6 text-clr1 rounded-xl text-xs font-bold hover:brightness-110 disabled:opacity-50"
                            >
                              {saving ? 'Guardando...' : '💾 Guardar'}
                            </button>
                            <button 
                              onClick={() => setEditingAcuerdoIdx(null)}
                              className="px-4 py-2 bg-clr7 dark:bg-dclr7 rounded-xl text-xs font-bold"
                            >
                              Cancelar
                            </button>
                          </div>
            ...
            <div className="space-y-10">
              <section className="space-y-4">
                ...
                <div className="space-y-2">
                  {acta.participantes?.map((p: any) => {
                    ...
                    return (
                      <div key={p.id} className="flex justify-between items-center text-[0.85em] font-bold uppercase p-2 bg-clr7 dark:bg-dclr7 rounded-[0.5em]">
                        <div className="truncate pr-2">
                          <p>{p.perfiles?.nombres}</p>
                          <p className="text-[0.8em] opacity-60">{p.rol_en_reunion}</p>
                        </div>
                        <span className={`whitespace-nowrap ${f?.firmado ? 'text-clr6' : 'text-clr5'}`}>
              ...
              <section className="space-y-4 bg-clr7 dark:bg-dclr7 p-2 rounded-[1rem] border dark:border-dclr7">
                ...
                <div className="space-y-4 text-[0.9em]">
                  {acta.proxima_reunion && (
                    <div>
                      ...
                      <p className="font-bold text-clr4">{acta.proxima_reunion}</p>
                    </div>
                  )}
                  <div>
        ...
        <button 
          onClick={onClose} 
          className="w-full py-4 bg-clr2 text-clr1 font-black uppercase rounded-[0.5em] mt-12 font-inika tracking-widest shadow-xl hover:brightness-125 transition-all text-[1em]"
        >
