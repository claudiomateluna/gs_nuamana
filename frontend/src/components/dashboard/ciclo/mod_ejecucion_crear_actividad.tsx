...
export default function DashModCrearActividadFase4({ isOpen, onClose, cicloId, perfilId, unidadNombre, onSuccess }: DashModCrearActividadFase4Props) {
  ...
  return (
    <div className="fixed inset-0 bg-pclr2 backdrop-blur-md z-[130] flex items-center justify-center p-2 animate-in fade-in duration-300">
      <div className="bg-pclr1 dark:bg-pdclr1 w-full max-w-2xl rounded-[2rem] p-4 md:p-4 shadow-2xl overflow-y-auto max-h-[90vh]">
        <h2 className="text-2xl font-black font-display uppercase text-pclr4 tracking-tighter mb-6">
          🆕 Nueva Actividad
        </h2>
        ...
        <form onSubmit={handleCrear} className="space-y-6">
          ...
          <div className="space-y-1">
            ...
            <textarea 
              ...
              className="w-full p-4 rounded-2xl border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 font-bold h-24"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full p-3 pl-4 rounded-xl border bg-pclr3 dark:bg-pdclr3 font-bold text-sm focus:ring-2 ring-clr6 outline-none transition-all"
              />
            </div>
                  ...
                  filteredActividades.map(art => (
                    ...
                    >
                      {art.imagen_destacada ? (
                        ...
                        <div className="w-12 h-12 rounded-xl bg-pclr3 dark:bg-pdclr3 flex items-center justify-center shrink-0">
                          <span className="text-xl">📋</span>
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold uppercase text-[0.85em] leading-tight text-pclr4 dark:text-pdclr4 truncate">{art.titulo}</h4>
                        <p className="text-[0.75em] opacity-60 italic line-clamp-1">{art.extracto}</p>
                      </div>
                      {selectedArticuloId === art.id && (
                        <span className="px-2 py-0.5 bg-pclr6 text-pclr12 text-[8px] font-black uppercase rounded-full tracking-widest shrink-0">
                          ✓
                        </span>
                      )}
                    </div>
