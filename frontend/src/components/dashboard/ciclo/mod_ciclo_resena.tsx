...
export default function DashModResena({ isOpen, onClose, propuesta, perfil, onSuccess }: DashModResenaProps) {
  ...
  const fetchResena = async () => {
    ...
    try {
      ...
      console.error('Error fetching resena:', getErrorMessage(err))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    if (!articuloId) { toast.info('Esta actividad no tiene una ficha vinculada para calificar.'); return; }
    setSaving(true)
    try {
      ...
      const unit = (perfil?.unidades || {}) as Record<string, unknown>
      const unitNameRaw = String((unit.nombre || '') as string).toLowerCase()
      ...
      const coloresObj = (unit.colores || {}) as Record<string, unknown>
      const unitColor = String(coloresObj.primario || '#cb3327')
      const unitLogo = String(unit.logo_unidad_url || '') || null
      ...
      toast.info('¡Gracias por tu reseña! Ayudará a otros scouts a conocer esta actividad.')
