    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-end">
        <div className="space-y-1">
          <h3 className="text-2xl font-black font-display uppercase tracking-tight text-clr4 dark:text-dclr4">Lluvia de Ideas</h3>
        ...
        {cicloActivo.fase_actual === 1 && !readOnlyOverride && (
          <button 
            ...
            className="p-2 text-clr1 font-bold uppercase rounded-2xl shadow-lg text-shadow-lg hover:scale-105 active:scale-95 transition-all text-base tracking-widest"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPropuestas.map(p => (
          <div 
            key={p.id} 
            className={`p-4 rounded-[1.5rem] bg-clr1 dark:bg-dclr1 border-2 transition-all shadow-sm flex flex-col justify-between gap-4 relative overflow-hidden ${
              p.preseleccionada || p.es_grupal_global ? "shadow-lg border-opacity-100" : "opacity-90 border-clr7 dark:border-dclr7"
            }`} 
            style={{ 
          ...
          >
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-start">
                ...
                {(p.preseleccionada || p.es_grupal_global) && (
                  <span className="px-2 py-0.5 bg-clr6 text-clr1 text-[1em] font-black rounded-md uppercase shadow-sm">
            ...
            {canManage && cicloActivo.fase_actual === 1 && !p.es_grupal_global && (
              <div className="pt-4 border-t border-clr7 dark:border-dclr7 flex justify-between items-center">
                <span className="text-[0.9em] font-black uppercase text-clr3">Gestión Admin</span>
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl text-[0.9em] font-bold text-shadow-lg uppercase transition-all shadow-sm ${
                      p.preseleccionada ? 'bg-clr7 text-clr3 hover:bg-clr7' : 'text-clr1 hover:brightness-110'
                    }`}
                  ...
                  <button 
                    ...
                    className="px-3 py-2 rounded-xl text-[0.9em] font-black uppercase bg-clr4 text-clr4 hover:bg-clr4 hover:text-clr1 transition-all shadow-sm"
