...
interface DashModEvaluarObjetivoProps {
  isOpen: boolean
  onClose: () => void
  perfil: any // El que está evaluando (Dirigente, Apoderado o el mismo NNJ)
  propuesta: any
  perfilNNJ?: any // El NNJ siendo evaluado (necesario si perfil es Dirigente/Apoderado)
  onSuccess?: () => void
}
...
export default function DashModEvaluarObjetivo({ isOpen, onClose, perfil, propuesta, perfilNNJ, onSuccess }: DashModEvaluarObjetivoProps) {
  ...
  const fetchObjetivos = async () => {
    ...
    try {
      ...
      if (targetNNJ.progresion_etapa_id) {
        ...
        if (targetNNJ.unidad_id === 1) { // Manada
          etapaRango = age >= 9 ? "Infancia Tardía" : "Infancia Media"
        } else if (targetNNJ.unidad_id === 2 || targetNNJ.unidad_id === 3) { // Compañía / Tropa
          etapaRango = age >= 13 ? "13 a 15 años" : "11 a 13 años"
        } else if (targetNNJ.unidad_id === 4) { // Avanzada
          etapaRango = "15 a 17 años"
        } else if (targetNNJ.unidad_id === 5) { // Clan
      ...
      console.error('Error fetching objetivos:', err.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  const guardarEvaluacion = async () => {
    if (objetivos.some(o => !respuestas[o.texto])) {
      alert(`Por favor, evalúa todos los ${term}s antes de guardar.`)
      return
    }
