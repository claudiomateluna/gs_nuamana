...
export default function DashModInventarioItem({ isOpen, onClose, onSuccess, editingItem, perfil, unidades }: DashModInventarioItemProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-2 md:p-4 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-dclr5 w-full max-w-3xl rounded-[2.5rem] p-6 md:p-10 shadow-2xl border-4 border-clr10 dark:border-dclr10 overflow-y-auto max-h-[95vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-[1em]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input required value={formData.nombre || ''} onChange={e => setFormData({...formData, nombre: e.target.value})} className="w-full p-4 rounded-2xl border bg-zinc-50 dark:bg-dclr10 font-bold" placeholder="Ej: Carpa técnica 4p..." />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[0.8em] font-bold uppercase opacity-40 ml-2">Categoría</label>
                <select value={formData.categoria || 'Otros'} onChange={e => setFormData({...formData, categoria: e.target.value})} className="w-full p-4 rounded-2xl border bg-zinc-50 dark:bg-dclr10 font-bold uppercase text-[0.9em]">
                  {CATEGORIAS.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-[0.8em] font-bold uppercase opacity-40 ml-2">Cantidad</label>
                <input type="number" min="1" value={formData.cantidad || 1} onChange={e => setFormData({...formData, cantidad: parseInt(e.target.value)})} className="w-full p-4 rounded-2xl border bg-zinc-50 dark:bg-dclr10 font-bold" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1">
              ...
              <select value={formData.estado || 'Disponible'} onChange={e => setFormData({...formData, estado: e.target.value})} className="w-full p-4 rounded-2xl border bg-zinc-50 dark:bg-dclr10 font-bold uppercase text-[0.9em]">
                {ESTADOS.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-[0.8em] font-bold uppercase opacity-40 ml-2">Condición Física</label>
              <select value={formData.condicion || 'Funcional'} onChange={e => setFormData({...formData, condicion: e.target.value})} className="w-full p-4 rounded-2xl border bg-zinc-50 dark:bg-dclr10 font-bold uppercase text-[0.9em]">
                {CONDICIONES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-[0.8em] font-bold uppercase opacity-40 ml-2">Asignación</label>
              <select value={formData.unidad_id || 'grupal'} onChange={e => setFormData({...formData, unidad_id: e.target.value})} className="w-full p-4 rounded-2xl border bg-zinc-50 dark:bg-dclr10 font-bold uppercase text-[0.9em]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-zinc-50 dark:bg-black/10 rounded-2xl">
            <div className="space-y-1">
              ...
              <select value={formData.origen || ''} onChange={e => setFormData({...formData, origen: e.target.value})} className="w-full p-3 rounded-xl border bg-white dark:bg-dclr10 font-bold uppercase text-[0.9em]">
            ...
            <div className="space-y-1">
              ...
              <input type="date" value={formData.fecha_adquisicion || ''} onChange={e => setFormData({...formData, fecha_adquisicion: e.target.value})} className="w-full p-3 rounded-xl border bg-white dark:bg-dclr10 font-bold" />
            </div>
            <div className="space-y-1">
              <label className="text-[0.8em] font-bold uppercase opacity-40">Caducidad (Alimentos)</label>
              <input type="date" value={formData.fecha_caducidad || ''} onChange={e => setFormData({...formData, fecha_caducidad: e.target.value})} className="w-full p-3 rounded-xl border bg-white dark:bg-dclr10 font-bold" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            {formData.tiene_garantia && (
              <div className="space-y-1 md:col-span-2 animate-in slide-in-from-left duration-300">
                ...
                <input type="date" required={formData.tiene_garantia} value={formData.fecha_garantia || ''} onChange={e => setFormData({...formData, fecha_garantia: e.target.value})} className="w-full p-3 rounded-xl border border-clr6 bg-white dark:bg-dclr10 font-bold" />
              </div>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            <textarea value={formData.descripcion || ''} onChange={e => setFormData({...formData, descripcion: e.target.value})} className="w-full p-3 rounded-xl border bg-zinc-50 dark:bg-dclr10 h-20 font-bold" placeholder="Escribe detalles adicionales..." />
          </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="button" onClick={onClose} className="px-8 py-5 bg-zinc-100 dark:bg-dclr1 text-clr2 font-bold uppercase rounded-[1.5rem] tracking-widest">
              Cancelar
            </button>
          </div>
        </form>
