...
4. **Categorías Jerárquicas (`articulo_categorias`), Tabla de 23 Categorías y Regla de Subcategoría Única**: 
   Cada artículo vincula su categoría específica e **implícitamente su categoría Padre**.
   
   **REGLA DE SUBCATEGORÍA ÚNICA (UN SÓLO HIJO POR RAÍZ):**
   Al asignar categorías en `articulo_categorias` (y en la insignia de la portada 5 capas), el artículo DEBE asociar su categoría Raíz (ej. `Actividades` ID: 1) y **ESTRICTAMENTE UNA SOLA categoría hijo (subcategoría)** directa (ej. o es `Juegos` ID: 7, o `Dinámicas` ID: 10, o `Juegos Nocturnos` ID: 9, o `Talleres` ID: 11, o `Juegos Democráticos` ID: 8). **QUEDA STRICTAMENTE PROHIBIDO asociar múltiples categorías hijo bajo la misma categoría raíz** (ej. NO se pueden asociar `Juegos` y `Dinámicas` a la vez, ni `Juegos` y `Juegos Nocturnos` a la vez). Se DEBE elegir la subcategoría más específica que defina la naturaleza de la actividad.
...
7. **`contenido` (HTML SEMÁNTICO LIMPIO)**: 
   ...
   - **PROHIBIDO MENCIONAR NOMBRES FUENTE U ORIGEN EN EL HTML O EXTRACTO:** Queda estrictamente prohibido incluir frases como *"originalmente conocido como..."*, *"adaptación de..."* o nombrar la fuente original dentro del `contenido` HTML o `extracto`. El texto debe presentar la actividad con su título institucional NuaMana de forma 100% limpia e independiente.
   - **EMBEBIDO DE CROQUIS Y ESQUEMAS TÁCTICOS:** Si la actividad original incluye un croquis, esquema, mapa o diagrama explicativo del trazado o funcionamiento del juego, SE DEBE extraer dicha imagen, guardarla optimizada en `frontend/public/uploads/[slug]-croquis.webp` e insertarla dentro del `contenido` HTML justo bajo `<h2>🎲 ¿Cómo se juega?</h2>` utilizando la estructura semántica:
     ```html
     <figure class="my-6 text-center">
       <img src="/uploads/[slug]-croquis.webp" alt="Esquema táctico del juego" class="mx-auto rounded-lg shadow-md max-w-full border border-gray-200" />
...
10. **`imagen_destacada` (ILUSTRACIÓN DESCRIPTIVA, PLANTILLA PROCESS_TEMPLATE_IMAGE 540x540 WebP, UNIFORME Y BANDERA NUA MANA)**: 
    ...
    - **ESTÁNDAR DE UNIFORME SCOUT NUA MANA Y PAÑOLÍN EN GENERATE_IMAGE:**
      ...
      - **REGLAS Y ROTACIÓN EQUITATIVA DE BANDERAS PERMITIDAS:**
        ...
        - **REGLA DE BANDERAS EN INTERIORES (INDOOR FLAG RULE):** En actividades que ocurren en recintos cerrados (`Interior`, `Salón`, `Sala`), **QUEDA ESTRICTAMENTE PROHIBIDO colocar mástiles o astas de exterior dentro de la habitación**. La bandera autorizada debe presentarse colgada de forma plana sobre la pared interior como un tapiz/estandarte (`"hanging flat as a wall banner on the interior wooden wall"`), o bien ser omitida.
...
3. **`cantidad` (MÍNIMO DE PARTICIPANTES OBLIGATORIO):** El campo `metadata.cantidad` DEBE apuntar **SIEMPRE a la cantidad MÍNIMA de participantes requeridos** para poder llevar a cabo la actividad (ej. si una dinámica se puede realizar desde 4 personas, se DEBE especificar `"04 participantes"`). Debe respetar siempre el formato cuantitativo oficial: dos dígitos con saltos de a 2 (`"02 participantes"`, `"04 participantes"`, `"06 participantes"`, `"08 participantes"`, `"10 participantes"`, `"12 participantes"`), o valores específicos (`"16 participantes"`, `"24 participantes"`, `"32 participantes"`, `"Toda la Unidad"`, `"individual"`).
4. **`lugares` (JERARQUÍA EXACTA LUGARES_TREE)**: Array de strings seleccionados respetando estrictamente el árbol jerárquico de la UI (`crear/page.tsx`):
   ```javascript
   // ÁRBOL OFICIAL DE LUGARES:
   // 1. Exterior
   //    ├── Campo Abierto ── [Bosque, Cerro, Río, Montaña]
   //    └── Campo Delimitado ── [Cancha, Piscina]
   // 2. Interior
   //    └── [Bus, Gimnasio, Sala, Salón]
   ```
   **Regla:** Incluir en el array toda la cadena de nodos seleccionados desde la raíz hasta el hijo específico:
   - Juego al aire libre en bosque: `["Exterior", "Campo Abierto", "Bosque"]`
   - Juego al aire libre en cancha delimitada: `["Exterior", "Campo Delimitado", "Cancha"]`
   - Juego en sala cerrada: `["Interior", "Sala"]`
   - Juego híbrido o múltiple: `["Exterior", "Interior", "Campo Delimitado", "Sala"]`
5. **`materiales` (ESTANDARIZADOS Y ATÓMICOS)**: Array de nombres **atómicos e independientes** (ej: `["Papel", "Lápiz"]` o `["Pelota"]` o `["Cuerda"]` o `["Sin Materiales"]`). **PROHIBIDO** incluir cadenas compuestas con conectores (como `["Papel y lápiz"]`) o especificaciones de marca/tamaño. Si se refiere a la prenda scout, usar siempre `"Pañolines"`.
...
11. **`objetivos_educativos` (COINCIDENCIA STRICTA 1:1, REGLA DE PARES POR UNIDAD Y BALANCEO DE EJES)**:
     - **CRITERIO DE BALANCEO Y PRIORIZACIÓN DE EJES DESATENDIDOS:** Al seleccionar los Ejes educativos (`texto_terminal`) para un nuevo artículo, la Skill DEBE priorizar aquellos Ejes de la categoría/área que posean menor o nula representación en la base de datos (especialmente los Ejes con 0 vinculaciones actuales o áreas como *Espiritualidad* y *Afectividad* cuando el contenido lo justifique).
     - **FLEXIBILIZACIÓN PEDAGÓGICA Y REGLA DE PARES INDIVISIBLES POR UNIDAD:**
       - Dentro de una misma Área de Desarrollo, distintas unidades (ej: Manada vs Tropa) PUEDEN utilizar Ejes Terminales (`texto_terminal`) diferentes si la madurez etaria de la unidad lo requiere.
       - **REGLA DE PARES INDIVISIBLES POR UNIDAD:** Para toda unidad declarada en `metadata.unidades`, se DEBEN incluir **SIEMPRE en par completo sus dos rangos de edad obligatorios** para el Eje Terminal seleccionado de esa unidad:
         - **Manada:** OBLIGATORIO incluir la pareja completa (*Infancia Media* + *Infancia Tardía*).
         - **Compañía:** OBLIGATORIO incluir la pareja completa (*11 a 13 años* + *13 a 15 años*).
         - **Tropa:** OBLIGATORIO incluir la pareja completa (*11 a 13 años* + *13 a 15 años*).
         - **Avanzada / Clan:** OBLIGATORIO incluir la pareja completa correspondiente según la malla curricular.
     - **REGLA DE COINCIDENCIA STRICTA DE UNIDADES:** `metadata.unidades` DEBE coincidir exactamente con las unidades incluidas en `metadata.objetivos_educativos`. Si una unidad figura en `metadata.unidades`, **ES OBLIGATORIO incluir sus objetivos educativos en pares**. Si la actividad no incluye objetivos para una unidad, esa unidad NO debe figurar en `metadata.unidades`.
...
## 🛡️ Reglas Anti-Duplicación y Verificación Previa (CRÍTICAS Y MANDATORIAS)
...
4. **Mapeo Curricular con la Matriz de 5 Claves y UUIDs Auténticos de BD (1:1 Unidades & Cobertura por Rango de Edad):** Consulta directamente `progresion_objetivos` en PostgreSQL obteniendo los UUIDs reales existentes (`id`), extrayendo el cruce de 5 claves agrupado por `texto_terminal` para TODAS las unidades declaradas en `metadata.unidades` (asegurando un `texto_infantil` por cada `rango_edad` de cada unidad), guarda `metadata.areas` estrictamente en minúsculas, y redacta los gerundios `como_se_cumple`.
