...
interface SpecialtyDetailProps {
  ...
  availableActivityArticles: any[]
  onClose: () => void
  onRefreshParent: () => Promise<void>
}

export default function DashmodProgresionEspecialidadDetalle({
  especialidad,
  userPerfil,
  perfil,
  availableMonitors,
  ...
  const [ep, setEp] = useState<any>(especialidad)
  ...
  useEffect(() => {
    if (ep) {
      ...
      ep.especialidades_actividades?.forEach((act: any) => {
  ...
  const isLeader = [1, 2, 3].includes(userPerfil.rol_id) && (userPerfil.rol_id === 1 || userPerfil.unidad_id === (ep.perfil?.unidad_id || perfil.unidad_id))
  ...
  const handleSaveExploracionChanges = async () => {
    if (!wDiagnostico.trim()) {
      alert('Por favor escribe tu exploración.')
      return
    }
    ...
    if (error) {
      ...
      alert('Exploración guardada con éxito.')
  ...
  const handleStartDevelopment = async () => {
    ...
    if (acts.length === 0) {
      alert('Por favor agrega al menos una tarea a realizar en tu planificación.')
      return
    }

    const missingDates = acts.some((a: any) => !taskDates[a.id])
    if (missingDates) {
      alert('Por favor establece una fecha límite para todas las tareas planificadas.')
      return
    }
    ...
    try {
      ...
      alert('¡Planificación guardada e inicio de Desarrollo exitoso! A trabajar en tus tareas.')
      await refreshLocalSpecialty()
    } catch (err: any) {
      console.error(err)
      alert('Error al iniciar desarrollo: ' + err.message)
    }
  }
    ...
    try {
      ...
      if (ep.monitor_perfil_id) {
        await supabase.from('notificaciones').insert([{
          ...
          mensaje: `${perfil.nombres} ha entregado evidencia para la tarea "${ep.especialidades_actividades?.find((a: any) => a.id === actId)?.descripcion || 'de especialidad'}" de "${name}". Revisa para validar.`,
          tipo: 'especialidad',
          link_url: '/dashboard'
        }])
      }
  ...
  const handleApproveActivity = async (actId: string) => {
    ...
    if (error) {
      ...
      const actDesc = ep.especialidades_actividades?.find((a: any) => a.id === actId)?.descripcion || 'Tarea'
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {acts.map((act: any) => (
                    <div 
                      key={act.id} 
                      className="p-4 rounded-2xl border border-zinc-150 dark:border-white/5 bg-white dark:bg-zinc-850 flex justify-between items-center"
                    >
                      <div className="space-y-1">
                        ...
                        {act.articulos && (
                          <div className="mt-0.5 flex items-center gap-1.5 text-[0.8em] font-semibold text-teal-600 dark:text-teal-400">
                            <span>📰 Artículo:</span>
                            <a 
                              href={`/blog/actividades/especialidades/${act.articulos.slug}`} 
                              target="_blank" 
                              rel="noreferrer" 
                          ...
                          <div className="p-3 bg-zinc-50 dark:bg-zinc-850/50 rounded-xl space-y-2 border">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="space-y-1">
                                ...
                                <select
                                  value={wNewActivityArticleId}
                                  onChange={e => {
                                    setWNewActivityArticleId(e.target.value);
                                    if (e.target.value) setNewActivityArticleTitle('');
                                  }}
                                  className="w-full p-2 border rounded-lg bg-white dark:bg-zinc-900 text-sm font-semibold"
                                >
                                  <option value="">-- No vincular artículo --</option>
                                  {availableActivityArticles.map(art => (
          ...
          {ep.fase === 'desarrollo' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {/* Progress bar */}
                <div className="w-full bg-zinc-100 dark:bg-zinc-850 h-3 rounded-full overflow-hidden border dark:border-zinc-800">
                    ...
                    <div 
                      ...
                      className={`p-4 rounded-3xl border transition-all ${
                        act.completada 
                          ...
                          : 'border-zinc-150 dark:border-white/5 bg-white dark:bg-zinc-850'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          ...
                          {act.fecha_limite && (
                            <span className="inline-block px-2.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-450 text-[0.8em] font-extrabold">
                              Límite: {new Date(act.fecha_limite + 'T00:00:00').toLocaleDateString('es-CL')}
                            </span>
                          )}
              ...
              {ep.estado === 'completado' && (
                <div className="p-5 rounded-3xl bg-green-50 dark:bg-green-950/20 border border-green-200 text-center space-y-4">
                  ...
                  <div className="flex justify-center">
                    <button
                      onClick={() => generateSpecialtyCertificate(ep.perfil || perfil, ep)}
