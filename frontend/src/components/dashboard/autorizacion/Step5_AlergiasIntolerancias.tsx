...
export default function Step5_AlergiasIntolerancias({ formData, setFormData, perfil }: StepProps) {
  ...
  const disabledInputStyle = "w-full bg-pclr3 dark:bg-pdclr3 p-3 rounded-xl font-bold text-[1em] dark:text-pdclr4 border border-pclr13 dark:border-pdclr13 opacity-50 cursor-not-allowed outline-none";
        ...
        <div className="md:col-span-2 space-y-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mt-2">
            {opcionesMenu.map(opt => {
              ...
              return (
                <label key={opt} className={`p-3 border-2 rounded-xl text-center cursor-pointer font-bold text-[0.8em] uppercase transition-all ${isChecked ? 'border-pclr14 bg-pclr10 text-pclr4' : 'border-pclr13 dark:border-pdclr13 dark:text-pdclr4'}`}>
