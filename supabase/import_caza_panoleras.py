...
# 3. Creatividad: "Actúa con agilidad mental ante las situaciones más diversas..."

raw_queries = [
    # --- UMBRELLA 1: CORPORALIDAD ---
    ("actividades deportivas y recreativas", "Manada", "Media", "Desplegando agilidad y reflejos rápidos al esquivar a mis compañeros en el campo de juego."),
    ("actividades deportivas y recreativas", "Manada", "Tardía", "Jugando con entusiasmo respetando las reglas de la caza de pañoletas."),
    ("actividades deportivas y recreativas", "Tropa", "11 a 13", "Ejercitando mi resistencia física y velocidad de reacción en el torneo de eliminación."),
    ...
    ("actividades deportivas y recreativas", "Compañía", "11 a 13", "Demostrando equilibrio y destreza corporal al capturar la pañoleta de mis oponentes."),
    ...
    # --- UMBRELLA 2: CARÁCTER ---
    ("sentido del humor", "Manada", "Media", "Disfrutando alegremente del juego sin molestarme si me quitan la pañoleta."),
    ...
    ("agilidad mental", "Compañía", "13 a 15", "Proponiendo tácticas creativas de protección de pañoleta en equipo."),
...
<p><strong>Caza de Pañoletas: Lucha de Exploradores</strong> es un vibrante y clásico juego scout de competencia física, velocidad, agilidad y reflejos. Los participantes se enfrentan en un cuadrilátero acotado intentando arrebatar la pañoleta colocada en la cintura del oponente mientras protegen la suya, fomentando el juego limpio y el trabajo en equipo.</p>

<hr>

<h3>🎲 ¿Cómo se juega?</h3>
<ol>
  <li><strong>Delimitación del Área:</strong> Se marca un espacio cuadrado en el césped o terreno plano. Todos los jugadores colocan un pañolín o pañoleta en la parte trasera de su cinturón, dejando colgar al menos dos tercios de la tela para que sea fácil de tomar.</li>
  <li><strong>Modalidad Individual (Todos contra Todos o Torneo):</strong> A la señal del dirigente, los jugadores se desplazan dentro del perímetro buscando quitar la pañoleta de sus oponentes sin golpear ni realizar agarres bruscos. Quien pierde su pañoleta queda temporalmente eliminado o entrega un punto.</li>
  <li><strong>Modalidad en Cuña (Ciempiés de Patrulla):</strong> La patrulla o equipo de 6 integrantes forma una fila tomada firmemente de la cintura. El último participante de la fila lleva la pañoleta. La patrulla debe moverse como un ciempiés coordinado para intentar quitar la pañoleta de la cuña rival sin desarmar su propia formación.</li>
  <li><strong>Cierre:</strong> Gana el participante o la patrulla que acumule más pañoletas al finalizar el tiempo estipulado.</li>
</ol>"""

variaciones_detalladas = """Pelea en el Tobillo: La pañoleta se ata suavemente alrededor de un tobillo. Esta variante exige mayor agilidad de piernas, giros rápidos y flexibilidad.

Desafío por Parejas Espalda con Espalda: Dos scouts se toman de los brazos por la espalda e intentan quitar las pañoletas de otras parejas sin romper el agarre de sus brazos.
...
recomendaciones_detalladas = """Seguridad y Juego Limpio: Los dirigentes deben recalcar que está estrictamente prohibido el juego rudo, empujones, zancadillas o agarres de ropa. Solo se permite el tirón limpio sobre la tela de la pañoleta.
...
BEGIN
  ...
  DELETE FROM articulos WHERE slug = 'caza-de-panoleras-lucha-de-exploradores';
  ...
  INSERT INTO articulos (
    ...
    v_admin_id,
    NULL,
    'Caza de Pañoletas: Lucha de Exploradores',
    'caza-de-panoleras-lucha-de-exploradores',
    $html${contenido_html_puro}$html$,
    'Un dinámico juego físico scout de agilidad, reflejos y juego limpio donde los competidores buscan arrebatar la pañoleta del oponente.',
    '/uploads/actividad_cazaPanoleras.webp',
    'publicado',
    ARRAY['juego-fisico', 'agilidad', 'panoleras', 'patrulla', 'exterior'],
...
with open("supabase/import_caza_panoleras.sql", "wb") as f:
    f.write(sql_script.encode("utf-8"))

print("Caza de Pañoletas SQL generated cleanly with authentic DB UUIDs.")
