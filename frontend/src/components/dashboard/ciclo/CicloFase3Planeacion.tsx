...
export default function CicloFase3Planeacion({
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className="bg-pclr1 dark:bg-pdclr1 rounded-3xl p-6 border border-pclr13 dark:border-pdclr13 shadow-sm mb-6">
        <div className="flex justify-between items-center mb-6">
          <button 
            type="button"
            onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} 
            className="w-8 h-8 flex items-center justify-center hover:bg-pclr3 dark:hover:bg-pdclr3 rounded-full transition-colors font-black"
          >
            ❮
          </button>
            ...
            return (
              <div 
                ...
                className={`aspect-square flex flex-col items-center justify-center rounded-2xl text-[0.8em] relative group transition-all ${
                  !isCurrentMonth ? 'text-pclr7 dark:text-pdclr7' : 'font-bold'
                } ${
                  isToday ? 'bg-pclr3 dark:bg-pdclr3 border-2 border-pclr13 dark:border-pdclr13' : 'border-2 border-transparent hover:border-pclr13 dark:hover:border-dclr7'
                } ${
                  allActivities.length > 0 ? 'cursor-pointer hover:scale-110 shadow-sm' : ''
                } ${hasGrupal ? 'ring-2 ring-pclr14 bg-pclr10' : ''}`}
              >
  ...
  return (
    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-1 text-center md:text-left">
        <h3 className="text-2xl font-black font-display uppercase tracking-tight text-pclr4 dark:text-pdclr4">Organización y Diseño</h3>
          ...
          <div className="grid gap-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {proposalsPending
              ...
              .map((p, index) => {
                ...
                return (
                  <div key={p.id} className="p-2 rounded-3xl bg-pclr1 dark:bg-pdclr1 border border-pclr13 dark:border-pdclr13 shadow-sm flex items-center justify-between gap-4 font-body">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl text-pclr12 shadow-inner shrink-0" style={{ backgroundColor: unitColor }}>
                        {index + 1}º
                      </div>
                      <div>
                        <h5 className="font-bold text-[1em] uppercase leading-tight text-pclr4 dark:text-pdclr4">{p.titulo}</h5>
                    ...
                    {canManage && cicloActivo.fase_actual === 3 && (
                      <button 
                        ...
                        className="p-2 bg-pclr10 text-pclr12 text-[0.9em] font-bold uppercase rounded-xl hover:scale-105 transition-all shadow-md shrink-0 border-none"
                      >
                        Agendar
                      </button>
                    )}
        ...
        <div className="space-y-2">
          ...
          <div className="grid gap-2">
            {proposalsScheduled
              ...
              .map(p => {
                ...
                <div key={p.id} className={`p-4 rounded-3xl border-2 shadow-sm flex flex-col gap-4 relative overflow-hidden group font-body ${
                  p.es_grupal_global 
                    ? 'border-pclr5' 
                    : 'bg-pclr1 dark:bg-pdclr1 border-pclr6'
                  ...
                  <div className="flex justify-between items-start z-10">
                    <div>
                      <span className={`text-[0.9em] font-bold uppercase ${p.es_grupal_global ? 'opacity-80' : 'text-pclr7'}`}>
                        {parseLocalDate(p.fecha_programada)?.toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' }) || '—'}
                      </span>
                      <h5 className={`font-bold text-lg uppercase leading-none mt-1 ${p.es_grupal_global ? '' : 'text-pclr4 dark:text-pdclr4'}`}>{p.titulo}</h5>
                          ...
                          {p.fichas_vinculadas.map((f: any) => (
                            <a key={f.id} href={`/blog/actividades/${f.slug}`} target="_blank"
                               className={`px-2 py-0.5 rounded-full text-[0.7em] font-bold ${
                                 p.es_grupal_global ? 'bg-pclr1 hover:bg-pclr1' : 'bg-pclr10 text-pclr4 hover:bg-pclr10'
                               }`}>
                              📋 {f.titulo}
                            </a>
                          ))}
                      ...
                      {p.es_especialidad && p.autor && (
                        <p className="text-[0.8em] font-semibold text-pclr4 dark:text-pdclr4 mt-1.5 uppercase tracking-wider">
                          Scout: {p.autor.nombres} {p.autor.apellidos}
                        </p>
                      )}
                      {p.es_grupal_global && (
                        <span className="inline-block mt-1 px-2 py-0.5 bg-pclr1 rounded-full text-[0.7em] font-bold uppercase">👥 Actividad Grupal</span>
                      )}
                    </div>
                  </div>

                  <div className="flex items-center justify-between z-10 border-t border-pclr1 pt-4">
                    {p.es_actividad_programada ? (
                      ...
                      <div className="flex items-center gap-3">
                        <span className="px-2 py-1 bg-pclr6 text-pclr12 dark:bg-pdclr6 dark:text-pdclr12 rounded-md text-[0.8em] font-bold uppercase">Planificada</span>
                        <a href={`/blog/actividades/${p.articulo.slug}`} target="_blank" className="text-[1em] font-bold underline hover:text-pclr4 transition-colors">
                          Ver Ficha
                        </a>
                      </div>
                    ) : p.es_grupal_global ? (
                      ...
                      <div className="flex items-center gap-2">
                        {p.fichas_vinculadas && p.fichas_vinculadas.length > 0 ? (
                          <span className="px-2 py-1 bg-pclr1 rounded-md text-[0.8em] font-bold uppercase">✅ Vinculada</span>
                        ) : (
                          <span className="px-2 py-1 bg-pclr1 rounded-md text-[0.8em] font-bold uppercase opacity-60">Sin ficha</span>
                        )}
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-1 bg-pclr5 text-pclr12 dark:bg-pdclr5 dark:text-pdclr12 rounded-md text-[0.8em] font-bold uppercase animate-pulse">Falta Ficha</span>
                        {canManage && cicloActivo.fase_actual === 3 && (
                          <button onClick={() => { setSelectedPropuesta(p); setIsModVincularArticuloOpen(true); }} className="text-[0.9em] font-bold text-pclr6 hover:underline uppercase border-none bg-transparent">
                            Vincular
                          </button>
                        )}
                      </div>
                    ...
                    <div className="flex gap-4 items-center">
                      {perfil?.unidad_id != null && (perfil.unidad_id === UNIT_IDS.AVANZADA || perfil.unidad_id === UNIT_IDS.CLAN) && !p.es_grupal_global && (
                        <button
                          ...
                          className="text-[0.8em] font-black uppercase text-pclr4 hover:text-pclr4 transition-colors border-none bg-transparent"
                        >
                      ...
                      {canManage && cicloActivo.fase_actual === 3 && !p.es_actividad_programada && !p.es_grupal_global && (
                        <button 
                          ...
                          className="text-[0.8em] opacity-60 hover:opacity-100 hover:text-pclr4 transition-colors uppercase font-bold border-none bg-transparent"
                        >
                          Desagendar
                        </button>
                      )}
            ...
            {proposalsScheduled.length === 0 && (
              <div className="py-20 text-center border-2 border-dashed border-pclr6 rounded-[2rem] opacity-60 bg-pclr6">
                <span className="text-4xl block mb-2">🗓️</span>
                <p className="text-[0.8em] font-black uppercase tracking-widest text-pclr6 dark:text-pdclr6">Aún no hay actividades agendadas</p>
              </div>
            )}
          </div>
        </div>
