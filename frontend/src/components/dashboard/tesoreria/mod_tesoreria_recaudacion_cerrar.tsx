...
import type { Perfil, TesoreriaItem } from '@/types'
...
export default function DashModRecaudacionCerrar({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error al cargar datos de cierre:', getErrorMessage(err))
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!itemIdIngreso) {
      toast.warning('Por favor seleccione el ítem contable para registrar el ingreso.')
      return
    }
    if (montoGasto !== '' && montoGasto > 0 && !itemIdEgreso) {
      toast.warning('Por favor seleccione el ítem contable para registrar el egreso del gasto.')
      return
    }
    ...
    try {
      ...
      toast.success('¡Recaudación completada y registrada en el Libro Diario exitosamente!')
      onSuccess()
      onClose()
    } catch (err: unknown) {
      toast.error('Error al cerrar recaudación: ' + getErrorMessage(err))
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-pclr2 backdrop-blur-sm z-[110] flex items-center justify-center p-2 md:p-4 animate-in fade-in duration-300">
      <div className="bg-pclr1 dark:bg-pdclr1 w-full max-w-lg rounded-[2.5rem] p-6 md:p-10 shadow-2xl border-4 border-pclr13 dark:border-pdclr13 overflow-y-auto max-h-[95vh]">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-black font-display uppercase text-pclr4 tracking-tighter">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-[1em]">
          <div className="bg-pclr3 dark:bg-pdclr3 p-5 rounded-2xl border text-center space-y-2">
            <p className="text-[0.8em] font-bold uppercase opacity-60">Total Validado Recaudado</p>
            <p className="text-3xl font-black text-pclr6">${totalRecaudado.toLocaleString('es-CL')}</p>
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full p-3 rounded-xl border bg-pclr3 dark:bg-pdclr3 font-bold uppercase text-[0.8em]"
            >
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full p-3 rounded-xl border bg-pclr3 dark:bg-pdclr3 font-black text-center text-[1.1em] text-pclr4"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t dark:border-pdclr13">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-6 py-3 bg-pclr10 text-pclr12 rounded-xl text-[0.85em] font-bold uppercase hover:brightness-110 shadow-lg transition-all disabled:opacity-50"
            >
