...
export default function DashModActaCrear({ isOpen, onClose, perfil, miembrosUnidad, onSuccess, editingActa }: DashModActaCrearProps) {
  ...
  return (
    <div className="fixed inset-0 bg-pclr2 backdrop-blur-sm z-[100] flex items-center justify-center p-2 animate-in fade-in duration-300 text-[1em]">
      <div className="bg-pclr1 dark:bg-pdclr1 w-full sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg rounded-l-[1rem] p-4 shadow-2xl overflow-y-auto max-h-[95vh]">
        <div className="flex justify-between items-center mb-4 border-b pb-4">
          <h2 className="text-3xl font-bold font-display uppercase text-pclr6 tracking-tighter">
            {editingActa ? `Editando: ${editingActa.codigo}` : 'Nueva Acta de Sesión'}
          </h2>
          <button onClick={onClose} className="text-2xl opacity-40 hover:opacity-100 hover:text-pclr4 font-bold">✕</button>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  ...
                  <select value={actaData.tipo} onChange={e => setActaData({...actaData, tipo: e.target.value})} className="w-full p-2 rounded-[0.6em] border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 uppercase text-[1em] font-bold">
                    {tiposDisponibles.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[0.8em] uppercase font-bold opacity-60">Fecha</label>
                  <input type="date" value={actaData.fecha} onChange={e => setActaData({...actaData, fecha: e.target.value})} className="w-full p-2 rounded-[0.6em] border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 text-[1em] font-bold" />
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-[0.8em] uppercase font-bold opacity-60">Confidencialidad</label>
                <select value={actaData.confidencialidad} onChange={e => setActaData({...actaData, confidencialidad: e.target.value})} className="w-full p-2 rounded-[0.6em] border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 text-[1em] uppercase font-bold">
              ...
              <div className="space-y-1">
                ...
                <textarea placeholder="Propósito de la sesión..." value={actaData.resumen} onChange={e => setActaData({...actaData, resumen: e.target.value})} className="w-full p-2 rounded-[0.6em] border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 h-24 text-[1em] font-bold" />
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b pb-2">
                ...
                <div className="relative">
                  <select 
                    ...
                    className="p-2 bg-pclr6 text-pclr12 rounded-xl text-[0.8em] font-bold uppercase cursor-pointer hover:brightness-110 shadow-md outline-none"
                  >
              ...
              <div className="grid grid-cols-1 gap-2 max-h-[350px] overflow-y-auto pr-2">
                {participantes.map(m => (
                  <div key={m.id} className="grid grid-cols-1 md:grid-cols-3 gap-2 p-2 bg-pclr3 dark:bg-pdclr3 rounded-[0.6em] border border-transparent hover:border-pclr6 relative group">
                    <button 
                      type="button" 
                      onClick={() => setParticipantes(participantes.filter(p => p.id !== m.id))}
                      className="absolute -top-1 -right-1 bg-pclr10 text-pclr12 w-5 h-5 rounded-full text-[0.8em] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10"
                    >
                      ✕
                    </button>
                    <span className="truncate text-[1em] self-center font-bold">{m.nombres} {m.apellidos}</span>
                    <select value={actaData.roles_participantes[m.id] || 'Asistente'} onChange={e => setActaData({...actaData, roles_participantes: {...actaData.roles_participantes, [m.id]: e.target.value}})} className="text-[0.8em] rounded-[0.6em] px-2 py-1 bg-pclr3 dark:bg-pdclr3 border dark:border-pdclr13 font-bold">
                    ...
                    <select value={actaData.asistencia[m.id] || 'Ausente'} onChange={e => setActaData({...actaData, asistencia: {...actaData.asistencia, [m.id]: e.target.value}})} className={`text-[0.8em] font-bold rounded-[0.6em] px-2 py-1 border-none uppercase ${
                      actaData.asistencia[m.id] === 'Presente' ? 'bg-pclr6 text-pclr12 dark:bg-pdclr6 dark:text-pdclr12' : 
                      actaData.asistencia[m.id] === 'No Invitado' ? 'bg-pclr3 text-pclr7' :
                      'bg-pclr10 text-pclr4'
                    }`}>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {actaData.temas.map((t: TemaData, i: number) => (
                <div key={i} className="p-2 bg-pclr3 dark:bg-pdclr3 rounded-[1rem] border dark:border-pdclr13 shadow-sm relative group">
                  <button type="button" onClick={() => setActaData((v: ActaData) => ({ ...v, temas: v.temas.filter((_: TemaData, idx: number)=>idx!==i)}))} className="absolute top-3 right-2 text-pclr4 font-bold opacity-60 hover:opacity-100">✕</button>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                    <div className="md:col-span-3 space-y-2">
                      <input placeholder="TÍTULO DEL TEMA" value={t.titulo} onChange={e => { const nt = [...actaData.temas]; nt[i].titulo = e.target.value; setActaData({...actaData, temas: nt}); }} className="w-full bg-transparent border-b-2 border-pclr13 font-bold text-[1.1em] pb-1 uppercase outline-none" />
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <textarea placeholder="Descripción / Agenda..." value={t.descripcion} onChange={e => { const nt = [...actaData.temas]; nt[i].descripcion = e.target.value; setActaData({...actaData, temas: nt}); }} className="w-full p-3 rounded-[0.5em] border dark:border-pdclr13 bg-pclr1 dark:bg-pdclr1 h-24 text-[1em] font-bold" />
                        <textarea placeholder="Conclusiones / Decisiones..." value={t.conclusiones} onChange={e => { const nt = [...actaData.temas]; nt[i].conclusiones = e.target.value; setActaData({...actaData, temas: nt}); }} className="w-full p-3 rounded-[0.5em] border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 border border-pclr6 dark:border-pdclr6 h-24 text-[1em] font-bold text-pclr4 dark:text-pdclr4" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="p-2 bg-pclr1 dark:bg-pdclr1 rounded-[0.5em] border dark:border-pdclr13">
                        <label className="text-[0.8em] uppercase opacity-60 font-bold">Duración Est.</label>
                        <input type="number" value={t.duracion_estimada} onChange={e => { const nt = [...actaData.temas]; nt[i].duracion_estimada = parseInt(e.target.value) || 0; setActaData({...actaData, temas: nt}); }} className="w-full bg-transparent dark:bg-pdclr3 font-bold" />
                      </div>
                      <div className="p-2 bg-pclr1 dark:bg-pdclr1 rounded-[0.5em] border dark:border-pdclr13">
                        <label className="text-[0.8em] uppercase opacity-60 font-bold">Duración Real</label>
                        <input type="number" value={t.duracion_real} onChange={e => { const nt = [...actaData.temas]; nt[i].duracion_real = parseInt(e.target.value) || 0; setActaData({...actaData, temas: nt}); }} className="w-full bg-transparent dark:bg-pdclr3 font-bold" />
                      </div>
                    </div>
                  </div>
                </div>
              ))}
              <button type="button" onClick={() => setActaData((v: ActaData) => ({ ...v, temas: [...v.temas, { titulo: '', descripcion: '', conclusiones: '', duracion_estimada: 15, duracion_real: 15 }] }))} className="w-full py-3 border-2 border-dashed border-pclr6 text-pclr6 rounded-[1rem] font-bold uppercase text-[1em] hover:bg-pclr6">+ Añadir Punto de Tabla</button>
            </div>
          </div>

          <div className="space-y-6">
            <h3 className="font-bold uppercase text-[1em] opacity-40 border-b pb-2 tracking-widest font-slab text-pclr4 dark:text-pdclr4">Acuerdos y Tareas</h3>
            <div className="space-y-4">
              {actaData.acuerdos.map((a: AcuerdoData, i: number) => (
                <div key={i} className="p-3 bg-pclr10 dark:bg-pdclr10 rounded-[1rem] border border-pclr14 dark:border-pdclr14 relative group grid grid-cols-1 md:grid-cols-4 gap-2">
                  <button type="button" onClick={() => setActaData((v: ActaData) => ({ ...v, acuerdos: v.acuerdos.filter((_: AcuerdoData, idx: number)=>idx!==i)}))} className="absolute top-4 right-6 text-pclr4 dark:text-pdclr4 font-bold opacity-40 dark:opacity-80 hover:opacity-100 font-display">✕</button>
                  <div className="md:col-span-2 space-y-3 font-bold">
                    <input placeholder="¿QUÉ SE HARÁ?" value={a.titulo} onChange={e => { const na = [...actaData.acuerdos]; na[i].titulo = e.target.value; setActaData({...actaData, acuerdos: na}); }} className="w-full bg-transparent border-b border-pclr14 font-bold text-[1em] pb-1 uppercase outline-none" />
                    <textarea placeholder="Detalles de la tarea..." value={a.descripcion} onChange={e => { const na = [...actaData.acuerdos]; na[i].descripcion = e.target.value; setActaData({...actaData, acuerdos: na}); }} className="w-full p-2 rounded-lg border dark:border-pdclr13 bg-pclr1 dark:bg-pdclr1 text-[1em] font-bold h-16" />
                  </div>
                  <div className="space-y-3">
                    <div>
                      <label className="text-[0.8em] uppercase opacity-40 font-bold">Responsable</label>
                      <select value={a.responsable_id || ''} onChange={e => { const na = [...actaData.acuerdos]; na[i].responsable_id = e.target.value; setActaData({...actaData, acuerdos: na}); }} className="w-full p-2 rounded-[0.5em] dark:border-pdclr13 bg-pclr1 dark:bg-pdclr1 border text-[0.8em] uppercase font-bold">
                        <option value="">Seleccionar...</option>
                        {allMiembros.map(m => (<option key={m.id} value={m.id}>{m.nombres} {m.apellidos}</option>))}
                      </select>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div><label className="text-[0.8em] uppercase opacity-40 font-bold">Plazo</label><input type="date" value={a.fecha_compromiso} onChange={e => { const na = [...actaData.acuerdos]; na[i].fecha_compromiso = e.target.value; setActaData({...actaData, acuerdos: na}); }} className="w-full p-2 dark:bg-pdclr3 rounded-[0.5em] border dark:border-pdclr13 text-[0.8em] font-bold" /></div>
                      <div><label className="text-[0.8em] uppercase font-bold">Prioridad</label><select value={a.prioridad} onChange={e => { const na = [...actaData.acuerdos]; na[i].prioridad = e.target.value; setActaData({...actaData, acuerdos: na}); }} className="w-full p-2 dark:bg-pdclr3 rounded-[0.5em] border dark:border-pdclr13 text-[0.8em] uppercase font-bold">{PRIORIDADES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <input 
                        ...
                        className="w-4 h-4 rounded border-pclr13 text-pclr4 focus:ring-pclr14"
                      />
                      <label htmlFor={`grupal-${i}`} className="text-[0.8em] font-black uppercase text-pclr4 dark:text-pdclr4 cursor-pointer">
                        ¿Actividad Grupal? 👥
                      </label>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-1">
                ...
                <input type="date" value={actaData.proxima_reunion} onChange={e => setActaData({...actaData, proxima_reunion: e.target.value})} className="w-full p-3 rounded-xl border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 font-bold" />
              </div>
              <div className="md:col-span-2 space-y-1">
                <label className="text-[0.8em] uppercase font-bold opacity-60">Observaciones Finales</label>
                <textarea placeholder="Comentarios de cierre..." value={actaData.observaciones_finales} onChange={e => setActaData({...actaData, observaciones_finales: e.target.value})} className="w-full p-2 rounded-xl border dark:border-pdclr13 bg-pclr3 dark:bg-pdclr3 h-24 text-[1em] font-bold" />
              </div>
            </div>
          </div>

          <div className="flex gap-2 pt-8 border-t">
            <button type="submit" disabled={saving} className="flex-1 py-4 bg-pclr6 text-pclr12 uppercase rounded-2xl shadow-xl font-inika font-bold tracking-widest text-[1em] hover:brightness-110">
              {saving ? '⌛ PROCESANDO...' : editingActa ? '💾 ACTUALIZAR LIBRO' : '💾 GUARDAR ACTA'}
            </button>
            <button type="button" onClick={onClose} className="px-10 py-4 bg-pclr3 dark:bg-pdclr3 rounded-[1em] font-bold uppercase tracking-widest text-[1em]">CANCELAR</button>
          </div>
        </form>
      </div>
    </div>
