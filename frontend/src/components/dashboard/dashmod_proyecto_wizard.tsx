...
import ProyectoPaso1Que from './proyecto/ProyectoPaso1Que'
import ProyectoPaso2PorQue from './proyecto/ProyectoPaso2PorQue'
import ProyectoPaso3ParaQue from './proyecto/ProyectoPaso3ParaQue'
import ProyectoPaso4ParaQuienes from './proyecto/ProyectoPaso4ParaQuienes'
import ProyectoPaso5Donde from './proyecto/ProyectoPaso5Donde'
import ProyectoPaso6Como from './proyecto/ProyectoPaso6Como'
import ProyectoPaso7Cuales from './proyecto/ProyectoPaso7Cuales'
import ProyectoPaso8Cuando from './proyecto/ProyectoPaso8Cuando'
import ProyectoPaso9Quienes from './proyecto/ProyectoPaso9Quienes'
import ProyectoPaso10Cuanto from './proyecto/ProyectoPaso10Cuanto'
import ProyectoPaso11ConQue from './proyecto/ProyectoPaso11ConQue'
import ProyectoPaso12ComoLoHicimos from './proyecto/ProyectoPaso12ComoLoHicimos'

interface ProyectoWizardProps {
  isOpen: boolean
  onClose: () => void
  perfil: any // Perfil del usuario actual
  proyecto?: any // Proyecto existente si estamos editando
  esGrupal: boolean // true = Colectivo (Unidad), false = Individual (Personal)
  onSuccess: () => void
  initialStep?: number
}
...
export default function DashModProyectoWizard({ isOpen, onClose, perfil, proyecto, esGrupal, onSuccess, initialStep }: ProyectoWizardProps) {
  ...
  const [articulosActividades, setArticulosActividades] = useState<any[]>([])
  const [activeCiclo, setActiveCiclo] = useState<any>(null)
  const [cicloActividades, setCicloActividades] = useState<any[]>([])
  const [otrosProyectosActividades, setOtrosProyectosActividades] = useState<any[]>([])

  // Colores de la Unidad
  const miUnidad = unidades.find(u => u.id === perfil?.unidad_id)
  const dbColors = miUnidad?.colores || perfil?.unidades?.colores || {}
  ...
  const [participantesManuales, setParticipantesManuales] = useState<any[]>([])
  ...
  const [financiamientoItems, setFinanciamientoItems] = useState<any[]>([])
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const profiles = await projectService.getProfiles()
  ...
  const updatePresupuestoItem = (index: number, field: string, value: any) => {
  ...
  const handleSave = async (concluir = false) => {
    if (!formData.titulo) return alert('Por favor, ingresa el título del proyecto.')
    setLoading(true)
    ...
    try {
      const payload: any = {
        ...
        es_grupal: esGrupal,
        campo_prioritario: perfil.unidad_id === 5 ? formData.campo_prioritario : null,
  ...
  const filteredMiembros = miembros.filter(m => {
    ...
    const matchesUnit = selectedUnitFilter === '' || (m.unidad_id !== null && m.unidad_id !== undefined && m.unidad_id.toString() === selectedUnitFilter)
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-full lg:max-w-[1024px] rounded-[1rem] shadow-2xl overflow-hidden flex flex-col my-auto max-h-[95vh] border-2 border-white/5 transition-all duration-300">
        ...
        <div className="relative overflow-hidden p-6 shadow-2xl border-b-4 flex flex-col md:flex-row items-center gap-6 transition-all shrink-0" style={{ backgroundColor: themePrimary, borderColor: themeSecondary }}>
          ...
          {(miUnidad?.logo_unidad_url || perfil?.unidades?.logo_unidad_url) && (
            <img 
              src={miUnidad?.logo_unidad_url || perfil?.unidades?.logo_unidad_url} 
              alt="" 
