...
export default function DashModActividadCrear({ isOpen, onClose, perfil, unidades, onSuccess, editingActividad }: DashModActividadCrearProps) {
  ...
  return (
    <div className="fixed inset-0 bg-pclr2 backdrop-blur-sm z-[150] flex items-center justify-center p-2 animate-in fade-in duration-300">
      <div className="bg-pclr1 dark:bg-pdclr1 w-full max-w-lg rounded-[2rem] p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
        <h2 className="text-2xl font-black font-display uppercase text-pclr6 mb-6 border-b pb-2 tracking-tighter">{isEditing ? 'Editar Actividad' : 'Programar Salida / Campamento'}</h2>
        ...
        <form onSubmit={handleSave} className="space-y-4 text-[1em]">
          <div className="grid grid-cols-2 gap-4">
            {editingActividad?.tipo === 'Especialidad' ? (
              <div className="space-y-1">
                ...
                <div className="w-full p-3 dark:bg-pdclr3 rounded-xl border font-bold uppercase bg-pclr3 text-pclr6">
                  🎓 Especialidad
                </div>
              </div>
            ) : (
              <div className="space-y-1">
                <label className="text-[0.8em] font-black uppercase opacity-40">Tipo</label>
                <select value={form.tipo} onChange={e => setForm({...form, tipo: e.target.value})} className="w-full p-3 dark:bg-pdclr10 rounded-xl border font-bold uppercase">
            ...
            {!isEditing && (
              <div className="space-y-1">
                ...
                <select value={form.nivel} onChange={e => setForm({...form, nivel: e.target.value})} className="w-full p-3 dark:bg-pdclr10 rounded-xl border font-bold uppercase">
          ...
          <div className="flex gap-4 pt-6 border-t mt-6">
            <button type="submit" disabled={loading} className="flex-1 py-4 bg-pclr6 text-pclr12 font-black uppercase rounded-2xl shadow-xl hover:brightness-110 transition-all font-inika tracking-widest">
              {loading ? '⌛ Guardando...' : isEditing ? '💾 Actualizar Actividad' : '💾 Crear Actividad'}
            </button>
            <button type="button" onClick={onClose} className="px-8 py-4 bg-pclr3 text-pclr7 font-bold uppercase rounded-2xl">Cancelar</button>
          </div>
        </form>
      </div>
    </div>
