...
const oldMapBody = `                {propuestas.filter(p => p.preseleccionada).map(p => {
                  const hasVoted = votos.some(v => v.propuesta_id === p.id && v.perfil_id === perfil.id)
                  const totalVotos = votos.filter(v => v.propuesta_id === p.id).length

                  return (
                    <div key={p.id} className={\`p-6 rounded-[2.5rem] bg-white dark:bg-black/20 border-2 transition-all shadow-sm flex flex-col justify-between gap-4 relative overflow-hidden \${hasVoted ? 'border-clr7' : 'border-zinc-100 dark:border-clr4'}\`}>
                      {hasVoted && <div className="absolute top-0 right-0 w-16 h-16 bg-clr7/10 rounded-bl-[100%] z-0" />}
                          ...
                          <button 
                            onClick={async () => {
                              if (hasVoted) {
                                ...
                                await supabase.from('ciclo_votos').insert([{ propuesta_id: p.id, perfil_id: perfil.id }])
                              }
                              fetchCiclo()
                            }}
                            className={\`px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm \${hasVoted ? 'bg-zinc-100 text-clr2 hover:bg-red-50 hover:text-red-500' : 'text-white hover:brightness-110 hover:scale-105 active:scale-95'}\`}
                            style={{ backgroundColor: hasVoted ? undefined : unitColor }}
                          >
                            {hasVoted ? 'Quitar Voto' : 'Votar por esto'}
                          </button>
                        )}
                      </div>
                    </div>
...
if (str.includes(oldMapBody)) {
  str = str.replace(oldMapBody, newMapBody);
  fs.writeFileSync(file, str, 'utf8');
  console.log('Update flexible voting UI successful');
} else {
  console.log('Old map body string not found exactly');
}
