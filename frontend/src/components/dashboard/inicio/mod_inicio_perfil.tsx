...
import { isAdmin, isDirigenteOrGuiadora, isNNJ, getRoleIds, Rol as RolId } from '@/lib/roles'
import type { Perfil, ContactoEmergencia, Unidad } from '@/types'
import { toast } from 'sonner';

interface ContactoEmergenciaLocal {
  id?: string
  perfil_id?: string
  nombre: string
  telefono: string
...
interface DashModPerfilProps {
  isOpen: boolean
  onClose: () => void
  editingPupilo: Perfil | null
  perfil: Perfil
  editData: Partial<Perfil> & { fecha_nacimiento_display?: string; nombre_apoderado_contacto?: string; relacion_apoderado_contacto?: string; telefono_apoderado_contacto?: string }
  setEditData: (data: (Partial<Perfil> & { fecha_nacimiento_display?: string; nombre_apoderado_contacto?: string; relacion_apoderado_contacto?: string; telefono_apoderado_contacto?: string }) | ((prev: Partial<Perfil> & { fecha_nacimiento_display?: string; nombre_apoderado_contacto?: string; relacion_apoderado_contacto?: string; telefono_apoderado_contacto?: string }) => Partial<Perfil> & { fecha_nacimiento_display?: string; nombre_apoderado_contacto?: string; relacion_apoderado_contacto?: string; telefono_apoderado_contacto?: string })) => void
  editContactos: ContactoEmergenciaLocal[]
  setEditContactos: (contacts: ContactoEmergenciaLocal[]) => void
...
export default function DashModPerfil({ 
  ...
  const [adultos, setAdultos] = useState<Pick<Perfil, 'id' | 'nombres' | 'apellidos' | 'rut'>[]>([])
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEditData((prev) => ({ ...prev, fecha_nacimiento_display: formatted }))
  }
  ...
  const handleSelectApoderado = (a: Pick<Perfil, 'id' | 'nombres' | 'apellidos'>) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Error desconocido del servidor'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  >
                    {roles.map(r => {
                      ...
                      const isDirOrGui = isDirigenteOrGuiadora(perfil);
                      const canManage = isAdminUser || isDirOrGui;
                      const isDisabled = (isOptionAdmin && !isAdminUser) || (!canManage && r.id !== editData.rol_id);
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {(isAdmin(perfil) || isDirigenteOrGuiadora(perfil)) ? (
                  <div className="space-y-1">
