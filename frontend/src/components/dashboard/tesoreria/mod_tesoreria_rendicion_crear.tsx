...
export default function DashModRendicionCrear({ isOpen, onClose, onSuccess, perfil, unidades }: DashModRendicionCrearProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[120] flex items-center justify-center p-1 md:p-2 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-dclr5 w-full max-w-5xl rounded-l-[1rem] p-2 md:p-4 shadow-2xl border-4 border-clr10 dark:border-dclr10 overflow-y-auto max-h-[95vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-[1em]">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 bg-zinc-50 dark:bg-black/10 p-4 rounded-[1rem] border">
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Responsable</label><input required value={form.nombre_responsable} onChange={e => setForm({...form, nombre_responsable: e.target.value})} className="w-full border border-clr3 dark:border-dclr2 p-2 rounded-xl bg-white dark:bg-dclr10 font-bold" /></div>
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Cargo</label><input required value={form.cargo_responsable} onChange={e => setForm({...form, cargo_responsable: e.target.value})} className="w-full border border-clr3 dark:border-dclr2 p-2 rounded-xl bg-white dark:bg-dclr10 font-bold" /></div>
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Fecha Reporte</label><input type="date" required value={form.fecha} onChange={e => setForm({...form, fecha: e.target.value})} className="w-full border border-clr3 dark:border-dclr2 p-2 rounded-xl bg-white dark:bg-dclr10 font-bold" /></div>
            <div className="md:col-span-2 space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Motivo / Actividad</label><input required value={form.motivo} onChange={e => setForm({...form, motivo: e.target.value})} className="w-full border border-clr3 dark:border-dclr2 p-2 rounded-xl bg-white dark:bg-dclr10 font-bold" placeholder="Ej: Campamento de Verano 2026" /></div>
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Unidad</label><select value={form.unidad_id} onChange={e => setForm({...form, unidad_id: e.target.value})} className="w-full border border-clr3 dark:border-dclr2 p-2 rounded-xl bg-white dark:bg-dclr10 font-bold uppercase"><option value="">⚜️ GRUPO</option>{unidades.map(u => <option key={u.id} value={u.id}>{u.nombre}</option>)}</select></div>
          </div>
            ...
            <div className="space-y-1 p-3 bg-zinc-50 dark:bg-black/10 rounded-[1rem] border relative">
              ...
              <div className="flex items-center gap-2">
                ...
                <input 
                  ...
                  className="w-full text-2xl font-black bg-transparent outline-none text-clr5 dark:text-dclr2" 
                />
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <input placeholder="Nombre Titular" value={form.banco_nombre_titular} onChange={e => setForm({...form, banco_nombre_titular: e.target.value})} className="p-3 rounded-xl border bg-zinc-50 dark:bg-dclr10 text-xs font-bold uppercase" />
              <input placeholder="RUT" value={form.banco_rut} onChange={e => setForm({...form, banco_rut: e.target.value})} className="p-3 rounded-xl border bg-zinc-50 dark:bg-dclr10 text-xs font-bold uppercase" />
              <input placeholder="Banco" value={form.banco_nombre_banco} onChange={e => setForm({...form, banco_nombre_banco: e.target.value})} className="p-3 rounded-xl border bg-zinc-50 dark:bg-dclr10 text-xs font-bold uppercase" />
              <input placeholder="Tipo Cuenta" value={form.banco_tipo_cuenta} onChange={e => setForm({...form, banco_tipo_cuenta: e.target.value})} className="p-3 rounded-xl border bg-zinc-50 dark:bg-dclr10 text-xs font-bold uppercase" />
              <input placeholder="Email" value={form.banco_email} onChange={e => setForm({...form, banco_email: e.target.value})} className="p-3 rounded-xl border bg-zinc-50 dark:bg-dclr10 text-xs font-bold" />
            </div>
          </div>

          <div className="flex gap-3 pt-4"><button type="submit" disabled={loading || selectedMovs.length === 0} className="flex-1 py-4 bg-clr6 text-white font-black uppercase rounded-2xl shadow-xl hover:brightness-110 tracking-widest transition-all disabled:opacity-30">{loading ? 'PROCESANDO...' : '💾 Generar Rendición Final'}</button><button type="button" onClick={onClose} className="px-8 py-4 bg-zinc-100 dark:bg-dclr1 text-clr2 font-bold uppercase rounded-2xl tracking-widest">CANCELAR</button></div>
        </form>
      </div>
    </div>
  )
