...
export default function DashModRendicionCrear({ isOpen, onClose, onSuccess, perfil, unidades }: DashModRendicionCrearProps) {
  ...
  return (
    <div className="fixed inset-0 bg-clr2 backdrop-blur-sm z-[120] flex items-center justify-center p-1 md:p-2 animate-in fade-in duration-300">
      <div className="bg-clr1 dark:bg-dclr1 w-full max-w-5xl rounded-l-[1rem] p-2 md:p-4 shadow-2xl border-4 border-clr7 dark:border-dclr7 overflow-y-auto max-h-[95vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-[1em]">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 bg-clr7 dark:bg-dclr7 p-4 rounded-[1rem] border">
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Responsable</label><input required value={form.nombre_responsable} onChange={e => setForm({...form, nombre_responsable: e.target.value})} className="w-full border border-clr2 dark:border-dclr2 p-2 rounded-xl bg-clr1 dark:bg-dclr1 font-bold" /></div>
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Cargo</label><input required value={form.cargo_responsable} onChange={e => setForm({...form, cargo_responsable: e.target.value})} className="w-full border border-clr2 dark:border-dclr2 p-2 rounded-xl bg-clr1 dark:bg-dclr1 font-bold" /></div>
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Fecha Reporte</label><input type="date" required value={form.fecha} onChange={e => setForm({...form, fecha: e.target.value})} className="w-full border border-clr2 dark:border-dclr2 p-2 rounded-xl bg-clr1 dark:bg-dclr1 font-bold" /></div>
            <div className="md:col-span-2 space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Motivo / Actividad</label><input required value={form.motivo} onChange={e => setForm({...form, motivo: e.target.value})} className="w-full border border-clr2 dark:border-dclr2 p-2 rounded-xl bg-clr1 dark:bg-dclr1 font-bold" placeholder="Ej: Campamento de Verano 2026" /></div>
            <div className="space-y-1"><label className="text-[0.8em] font-black uppercase opacity-80">Unidad</label><select value={form.unidad_id} onChange={e => setForm({...form, unidad_id: e.target.value})} className="w-full border border-clr2 dark:border-dclr2 p-2 rounded-xl bg-clr1 dark:bg-dclr1 font-bold uppercase"><option value="">⚜️ GRUPO</option>{unidades.map(u => <option key={u.id} value={u.id}>{u.nombre}</option>)}</select></div>
          </div>

          <div className="space-y-2">
            <div className="flex justify-between items-center"><h3 className="text-[0.8em] font-black uppercase opacity-80 tracking-widest">Seleccionar Movimientos a Rendir</h3><span className="text-[0.8em] bg-clr6 text-clr1 px-3 py-1 rounded-full font-bold">{selectedMovs.length} Seleccionados</span></div>
            <div className="border rounded-[1rem] overflow-hidden max-h-[300px] overflow-y-auto"><table className="w-full text-left text-[0.8em]"><thead className="sticky top-0 bg-clr7 dark:bg-dclr7 font-black uppercase"><tr><th className="p-3 w-10"></th><th className="p-3 w-24">Fecha</th><th className="p-3">Descripción</th><th className="p-3 text-right">Monto</th></tr></thead><tbody>{availableMovs.map(m => (<tr key={m.id} onClick={() => toggleMov(m.id)} className={`border-t cursor-pointer hover:bg-clr7 dark:hover:bg-dclr7 ${selectedMovs.includes(m.id) ? 'bg-clr6 border-l-4 border-l-clr6' : ''}`}><td className="p-3 text-center"><input type="checkbox" checked={selectedMovs.includes(m.id)} readOnly className="w-4 h-4 rounded accent-clr6" /></td><td className="p-3 font-bold">{new Date(m.fecha_completa).toLocaleDateString('es-CL')}</td><td className="p-3"><p className="font-bold uppercase">{m.descripcion}</p><p className="opacity-80 text-[1em]">{m.tesoreria_items?.codigo} {m.tesoreria_items?.nombre}</p></td><td className="p-3 text-right font-black">${(m.monto_egreso || m.monto_ingreso).toLocaleString('es-CL')}</td></tr>))}</tbody></table></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1 p-3 bg-clr7 dark:bg-dclr7 rounded-[1rem] border relative">
              ...
              <div className="flex items-center gap-2">
                ...
                <input 
                  ...
                  className="w-full text-2xl font-black bg-transparent outline-none text-clr4 dark:text-dclr4" 
                />
              </div>
            ...
            <div className="space-y-1 p-3 bg-clr7 dark:bg-dclr7 rounded-[1rem] border">
              <label className="text-[0.8em] font-black uppercase opacity-40">TOTAL RENDICIÓN $</label>
              <p className="text-2xl font-black text-clr4">${totalRendicion.toLocaleString('es-CL')}</p>
            </div>
            <div className={`space-y-1 p-3 rounded-[1rem] border shadow-xl transition-colors ${saldoFinal >= 0 ? 'bg-clr6 text-clr1' : 'bg-clr4 text-clr1'}`}>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <input placeholder="Nombre Titular" value={form.banco_nombre_titular} onChange={e => setForm({...form, banco_nombre_titular: e.target.value})} className="p-3 rounded-xl border bg-clr7 dark:bg-dclr7 text-xs font-bold uppercase" />
              <input placeholder="RUT" value={form.banco_rut} onChange={e => setForm({...form, banco_rut: e.target.value})} className="p-3 rounded-xl border bg-clr7 dark:bg-dclr7 text-xs font-bold uppercase" />
              <input placeholder="Banco" value={form.banco_nombre_banco} onChange={e => setForm({...form, banco_nombre_banco: e.target.value})} className="p-3 rounded-xl border bg-clr7 dark:bg-dclr7 text-xs font-bold uppercase" />
              <input placeholder="Tipo Cuenta" value={form.banco_tipo_cuenta} onChange={e => setForm({...form, banco_tipo_cuenta: e.target.value})} className="p-3 rounded-xl border bg-clr7 dark:bg-dclr7 text-xs font-bold uppercase" />
              <input placeholder="Email" value={form.banco_email} onChange={e => setForm({...form, banco_email: e.target.value})} className="p-3 rounded-xl border bg-clr7 dark:bg-dclr7 text-xs font-bold" />
            </div>
          </div>

          <div className="flex gap-3 pt-4"><button type="submit" disabled={loading || selectedMovs.length === 0} className="flex-1 py-4 bg-clr6 text-clr1 font-black uppercase rounded-2xl shadow-xl hover:brightness-110 tracking-widest transition-all disabled:opacity-30">{loading ? 'PROCESANDO...' : '💾 Generar Rendición Final'}</button><button type="button" onClick={onClose} className="px-8 py-4 bg-clr7 dark:bg-dclr7 text-clr3 font-bold uppercase rounded-2xl tracking-widest">CANCELAR</button></div>
        </form>
      </div>
    </div>
  )
