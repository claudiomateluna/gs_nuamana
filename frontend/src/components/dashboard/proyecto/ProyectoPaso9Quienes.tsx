  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1 p-2 opacity-60 rounded-[0.8rem]" style={{ backgroundColor: themePrimary }}>
        <span className="w-8 h-8 rounded-full flex items-center justify-center text-pclr12 text-[0.9em] font-bold" style={{ color: themeSecondary }}>9</span>
        <h3 className="text-xl font-black uppercase tracking-tight text-pclr4 dark:text-pdclr4" style={{ color: themeSecondary }}>¿Quiénes lo Haremos?</h3>
      </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 min-h-[420px] p-1 font-body">
        ...
        <div className="w-full space-y-2 bg-pclr3 dark:bg-pdclr3 p-1 rounded-[1rem] border border-pclr13 dark:border-pdclr13 backdrop-blur-xs shadow-md">
          ...
          <div className="space-y-2 bg-pclr1 dark:bg-pdclr1 p-2 rounded-2xl border dark:border-pclr1">
            <span className="text-[0.8em] font-black uppercase tracking-wider text-pclr7 block">Miembros del Grupo</span>
            ...
            <div className="flex flex-col gap-1.5">
              <input 
                ...
                placeholder="Buscar..."
                className="w-full p-2 rounded-xl border dark:border-pclr1 bg-pclr3 dark:bg-pdclr3 text-[0.8em] font-bold outline-none"
              />
              <select
                value={selectedUnitFilter}
                onChange={e => setSelectedUnitFilter(e.target.value)}
                className="w-full p-2 rounded-xl border dark:border-pclr1 bg-pclr3 dark:bg-pdclr3 text-[0.9em] font-bold tracking-tight outline-none"
              >
            ...
            <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1">
              {filteredMiembros.map(m => {
                ...
                return (
                  <div key={m.id} className="flex flex-col gap-1 p-2 bg-pclr3 dark:bg-pdclr3 rounded-xl border dark:border-pclr1">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input 
                        type="checkbox" 
                        checked={isSelected}
                        onChange={() => handleToggleParticipanteReg(m.id)}
                        className="rounded border-pclr13 text-pclr4 focus:ring-pclr14"
                      />
                      <span className="text-[1em] text-pclr4 dark:text-pdclr4">
                    ...
                    {isSelected && (
                      <div className="flex flex-col gap-1">
                        <select
                          ...
                          className="w-full text-[0.8em] font-bold uppercase tracking-tight bg-pclr1 dark:bg-pdclr1 p-1.5 rounded-lg border dark:border-pclr1 outline-none"
                        >
                        ...
                        <div className="flex flex-wrap gap-1 mt-1 p-1.5 bg-pclr1 dark:bg-pdclr1 rounded-xl border dark:border-pclr1">
                          <span className="w-full text-[0.8em] font-black uppercase text-pclr7 mb-0.5 ml-1 text-left">Tareas Asignadas:</span>
                          {actividades.map(act => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-2 py-0.5 rounded-full text-[0.8em] font-bold uppercase transition-all border ${
                                  isAssigned 
                                    ? 'text-pclr12 border-transparent' 
                                    : 'bg-pclr1 hover:bg-pclr3 dark:bg-pdclr1 dark:hover:bg-pdclr1 text-pclr7 dark:text-pdclr7 border-pclr13 dark:border-pdclr13'
                                }`}
                          ...
                          {actividades.length === 0 && (
                            <span className="text-[0.8em] italic text-pclr7 ml-1">No hay actividades en Paso 7.</span>
                          )}
                        </div>
                      </div>
                    )}
          ...
          <div className="space-y-2 bg-pclr1 dark:bg-pdclr1 p-2 rounded-2xl border dark:border-pclr1">
            <div className="flex justify-between items-center">
              <span className="text-[0.8em] font-black uppercase tracking-wider text-pclr7">Externos / Colaboradores</span>
              <button 
                type="button" 
            ...
            <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
              {participantesManuales.map((p, idx) => (
                <div key={idx} className="p-2 bg-pclr3 dark:bg-pdclr3 rounded-xl border dark:border-pclr1 space-y-1.5 relative">
                  <button 
                    type="button" 
                    onClick={() => removeParticipanteManual(idx)}
                    className="absolute top-1 right-2 text-pclr4 font-bold hover:bg-pclr10 dark:hover:bg-dclr4 px-1 rounded text-[0.85em] border-none bg-transparent"
                  >
                    ✕
                  </button>
                  <input 
                    ...
                    className="w-full p-1.5 rounded-lg border dark:border-pclr1 bg-pclr1 dark:bg-pdclr1 text-[0.8em] font-bold"
                  />
                  <div className="flex flex-col sm:flex-row gap-1.5">
                    <input 
                      ...
                      className="w-full sm:w-1/2 p-1.5 rounded-lg border dark:border-pclr1 bg-pclr1 dark:bg-pdclr1 text-[0.8em] font-bold text-center"
                    />
                    <input 
                      type="email" 
                      value={p.email || ''}
                      ...
                      placeholder="Email"
                      className="w-full sm:w-1/2 p-1.5 rounded-lg border dark:border-pclr1 bg-pclr1 dark:bg-pdclr1 text-[0.8em] font-bold"
                    />
                  </div>
                  <div className="flex flex-wrap gap-1 mt-1 p-1.5 bg-pclr1 dark:bg-pdclr1 rounded-xl border dark:border-pclr1">
                    <span className="w-full text-[0.8em] font-black uppercase text-pclr7 mb-0.5 ml-1 text-left">Tareas Asignadas:</span>
