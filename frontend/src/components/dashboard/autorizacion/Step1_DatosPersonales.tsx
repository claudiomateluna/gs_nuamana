        ...
        <Field fullWidth label="¿Cuál es la asignación Femenina/Masculina entregada al nacer?" info="Selecciona la asignación femenina/masculina entregada al nacer de la persona que está completando el formulario. *Este Dato es Obligatorio">
          <div className="flex gap-6">
            {['Femenina', 'Masculina'].map(s => {
              ...
              return (
                <label key={s} className={`flex-1 p-3 border-2 rounded-xl text-center cursor-pointer font-bold uppercase text-[0.9em] tracking-widest transition-all ${isChecked ? 'border-clr4 bg-clr4 text-clr1 shadow-lg scale-105' : 'border-clr7 dark:border-dclr7 dark:text-dclr2'}`}>
