...
export default function DashModRecaudacionComprobante({
  ...
  const sortedUsers = useMemo(() => {
    ...
    const otherNNJ = users
      .filter(u => u.id !== perfil.id && u.apoderado_id !== perfil.id && u.rol_id >= 9 && u.rol_id <= 13)
      .sort((a, b) => `${a.nombres} ${a.apellidos}`.localeCompare(`${b.nombres} ${b.apellidos}`))

    const leaders = users
      .filter(u => u.id !== perfil.id && (u.rol_id === 2 || u.rol_id === 3))
      .sort((a, b) => `${a.nombres} ${a.apellidos}`.localeCompare(`${b.nombres} ${b.apellidos}`))

    const parents = users
      .filter(u => u.id !== perfil.id && u.rol_id >= 4 && u.rol_id <= 8)
      .sort((a, b) => `${a.nombres} ${a.apellidos}`.localeCompare(`${b.nombres} ${b.apellidos}`))

    const others = users
      .filter(u => u.id !== perfil.id && u.apoderado_id !== perfil.id && u.rol_id > 13)
      .sort((a, b) => `${a.nombres} ${a.apellidos}`.localeCompare(`${b.nombres} ${b.apellidos}`))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedUserIds.length === 0) {
      alert('Debe seleccionar al menos un participante (NNJ/miembro) asociado al pago.')
      return
    }
    if (!monto || monto <= 0) {
    ...
    if (!archivo) {
      alert('Debe adjuntar la imagen o PDF del comprobante de transferencia.')
      return
    }
    ...
    try {
      ...
      alert('¡Comprobante enviado con éxito! Queda pendiente de validación por parte del equipo de tesorería.')
      onSuccess()
      onClose()
    } catch (err: any) {
              ...
              {loading ? (
                ...
                <div className="divide-y divide-zinc-100 dark:divide-white/5">
                  {filteredUsers.map(u => {
                    ...
                    else if (u.rol_id >= 9 && u.rol_id <= 13) badge = '👤 NNJ';
                    else if (u.rol_id === 2 || u.rol_id === 3) badge = '⚜️ Dirigente';
