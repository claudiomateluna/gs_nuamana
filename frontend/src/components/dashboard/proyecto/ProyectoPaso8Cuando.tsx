      ...
      <div className="bg-pclr1 dark:bg-pdclr1 rounded-3xl p-4 border border-pclr13 dark:border-pdclr13 shadow-inner w-full">
        <div className="flex justify-between items-center mb-4">
          <button 
            ...
            className="w-7 h-7 flex items-center justify-center hover:bg-pclr3 dark:hover:bg-pdclr3 rounded-full transition-colors text-[0.9em] font-bold text-pclr7 border-none bg-transparent"
          >
            ❮
          </button>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1 p-2 opacity-60 rounded-[0.8rem]" style={{ backgroundColor: themePrimary }}>
        <span className="w-8 h-8 rounded-full flex items-center justify-center text-pclr12 text-[0.9em] font-bold" style={{ color: themeSecondary }}>8</span>
        <h3 className="text-xl font-black uppercase tracking-tight text-pclr4 dark:text-pdclr4" style={{ color: themeSecondary }}>¿Cuándo?</h3>
      </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 min-h-[420px] p-1 font-body">
        ...
        <div className="w-full space-y-2 bg-pclr3 dark:bg-pdclr3 p-1 rounded-[1rem] border border-pclr13 dark:border-pdclr13 backdrop-blur-xs shadow-md">
          ...
          <p className="text-[0.8em] text-pclr7 font-bold p-1 leading-tight mt-2">
            Calendariza las actividades. Se reflejarán en el calendario de la unidad.
          </p>

          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
            {actividades.map((act, index) => (
              <div key={act.id} className="p-2 bg-pclr1 dark:bg-pdclr1 border dark:border-pclr1 rounded-xl flex flex-col gap-2">
                <div>
                  <span className="text-[0.8em] font-black uppercase text-pclr7">Actividad #{index + 1}</span>
                  <h5 className="font-bold text-[0.85em] text-pclr4 dark:text-pdclr4 uppercase leading-none">{act.nombre || '(Sin Nombre)'}</h5>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input 
                    ...
                    className="flex-1 p-2 bg-pclr3 dark:bg-pdclr3 rounded-xl border dark:border-pclr1 text-[0.85em] font-bold text-center"
                  />
                </div>
              </div>
            ))}
            ...
            {actividades.length === 0 && (
              <p className="text-center py-6 text-pclr7 font-bold italic text-[0.85em]">Debes agregar actividades en el Paso 7 antes de calendarizar.</p>
            )}
          </div>
        </div>
      </div>
